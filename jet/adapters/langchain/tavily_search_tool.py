...
class TavilySearchResults(BaseTool):
    ...
    api_wrapper: TavilySearchAPIWrapper = Field(
        default_factory=TavilySearchAPIWrapper)  # type: ignore[arg-type]
    response_format: Literal["content_and_artifact"] = "content_and_artifact"
    redis_config: RedisConfigParams = Field(
        default_factory=lambda: {"port": DEFAULT_REDIS_PORT},
        description="Redis configuration parameters"
    )
    cache: RedisCache = Field(
        default_factory=lambda: RedisCache(
            config={"port": DEFAULT_REDIS_PORT}),
        description="Redis cache instance for storing search results"
    )
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        # Update redis_config with provided values
        redis_config: RedisConfigParams = {
            "port": DEFAULT_REDIS_PORT, **kwargs.get("redis_config", {})}
        kwargs["redis_config"] = redis_config
        # Initialize cache with updated redis_config
        kwargs["cache"] = RedisCache(config=redis_config)
