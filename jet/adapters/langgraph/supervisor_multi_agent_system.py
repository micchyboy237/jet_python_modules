...
# ─── Observability (mirrors your existing setup) ────────────────────────────
console = Console()
logging.basicConfig(
    level=logging.INFO,
    format="%(message)s",
...
# ─── Mem0 Setup (reuses your existing config pattern) ───────────────────────
# Replace with your actual factory call: get_memory_config("supervisor_memories_v1")
# MEM0_CONFIG = {
#     "vector_store": {
#         "provider": "qdrant",
#         "config": {"collection_name": "supervisor_demo", "embedding_model_dims": 1536},
#     }
# }
...
# ─── LLM Factory (replace with your jet adapter if needed) ──────────────────
...
class SupervisorState(TypedDict):
    messages: Annotated[
        list, lambda x, y: x + y
    ]  # Simple append; consolidation handles size
    next_agent: str
    user_id: str
    consolidated_context: (
        str  # Compressed history injected into every worker/supervisor call
    )
...
def consolidate_messages(messages: list[BaseMessage], max_raw_messages: int = 8) -> str:
    ...
    formatted = "\n".join(
        f"[{getattr(m, 'name', m.type)}]: {m.content[:500]}" for m in older if m.content
    )
...
# ─── Worker Agents with Iterative Self-Evaluation ───────────────────────────
...
# Note: In production, bind real tools (DuckDuckGoSearchRun, PythonREPLTool, etc.)
# Using empty tools here for demo portability; add your tools to the lists below.
...
# ─── Worker Wrapper Nodes (Consolidation + Observability) ───────────────────
def _worker_node(state: SupervisorState, agent_name: str, agent_graph) -> dict:
    ...
    with tracer.start_as_current_span(
        ...
        # Inject consolidated context so worker has history without token bloat
        augmented_messages = []
        ...
        # Extract ONLY the final synthesized message (discard internal iteration)
...
def supervisor_node(state: SupervisorState) -> dict:
    with tracer.start_as_current_span(
        ...
        # Run consolidation BEFORE supervisor decides routing
        ...
        messages_for_routing.extend(state["messages"][-4:])  # Recent messages verbatim
...
if __name__ == "__main__":
    ...
    for msg in result["messages"]:
        ...
        content = msg.content[:600] + ("..." if len(str(msg.content)) > 600 else "")
