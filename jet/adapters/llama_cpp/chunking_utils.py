...
def truncate_texts(
    ...
    This is the recommended implementation based on text_chunker.py's truncate_texts_fast,
    adapted to use the chunking_utils tokenization ecosystem (HuggingFace tokenizer).

    Key improvements over the original truncate_texts:
    - Uses split_sentences_with_separators for single-pass sentence splitting with separators
    - No fragile text.find() position hunting
    - Simple "".join() reconstruction instead of parallel sentence/separator arrays
    - Optional progress bar for batch processing
    - Returns single string if input is string, list if input is list
    ...
    Args:
        ...
        show_progress: Show progress bar during processing (only for list inputs).

    Returns:
        Truncated text string (if input was str) or list of truncated strings (if input was list).
        Empty strings are filtered out for list inputs; single string input may return empty string.
    """
    ...
    # Only show progress bar for batch processing
    ...
    for text in text_iter:
        ...
        for sentence in sentences:
            ...
            if current_tokens + sentence_len > max_tokens:
                logger.debug(
                    ...
                    f"next sentence would add {sentence_len} tokens"
                )
                break
        ...
        if kept_sentences:
            # Reconstruct text by joining sentences (separators already included)
            truncated = "".join(kept_sentences).strip()
            results.append(truncated)
            logger.debug(
                f"Truncated text from {len(tokenizer.encode(text, add_special_tokens=False))} "
                f"to {current_tokens} tokens, {len(kept_sentences)} sentences kept"
            )
        else:
            # If no complete sentence fits, take at least something
            logger.debug(
                "No complete sentence fits within token limit, truncating first sentence"
            )
            if sentences:
    ...
    logger.info(
        f"Truncated {len(texts)} text(s) - "
        f"non-empty results: {sum(1 for r in results if r)}/{len(results)}"
    )
