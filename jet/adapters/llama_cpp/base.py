...
from jet.adapters.llama_cpp.log_utils import (
    console,
    get_file_logger,
    make_call_dir,
    print_content_chunk,
...
class Llama(BaseLlama):
    ...
    def create_chat_completion(
        ...
        if not stream:
            ...
            console.print(
                f"[dim]Response → [link={file_uri}]{response_md}[/link][/dim]"
            )
            save_json(
        ...
        def _stream_wrapper(
            ...
            console.print("[bold blue]⟶ Streaming…[/bold blue]", highlight=False)
            ...
            save_markdown(
                call_dir / "response.md", full_content, full_reasoning or None
            )
            response_md = call_dir / "response.md"
            file_uri = response_md.resolve().as_uri()
            logger.info("Response saved  path=%s  uri=%s", response_md, file_uri)
