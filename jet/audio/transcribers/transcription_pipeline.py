...
class TranscriptionPipeline:
    def __init__(
        ...
        self._cache: dict[AudioKey, tuple[str, str]] = {}
    ...
    def _make_key(self, audio: NDArray[np.float32]) -> AudioKey:
        # Fast perceptual-ish hash – good enough to avoid exact duplicate segments
        duration = len(audio) / 16000.0
        h = hash(audio.tobytes())
        return AudioKey(hash=h, duration_sec=round(duration, 3))

    def _cache_get(self, key: AudioKey) -> Optional[tuple[str, str]]:
        with self._lock:
            return self._cache.get(key)

    def _cache_set(self, key: AudioKey, ja: str, en: str) -> None:
        with self._lock:
            logger.debug("CACHE SET attempt → key=%s ja='%s' en='%s'", key, ja, en)
            if key in self._cache:
                logger.debug("  → key already exists, removing from order")
                self._cache_order.remove(key)
            self._cache[key] = (ja, en)
    ...
    def submit_segment(self, audio: NDArray[np.float32]) -> None:
        ...
        if cached := self._cache_get(key):
            ja_text, en_text = cached
            logger.debug("CACHE HIT → immediate print")
