    ...
    for seg in segments:
        ...
        for config in limit_configs:
            pieces, trough = _apply_single_limit_split(
                ...
                hop_sec=hop_sec,
                config=config,
                max_half_limit=max_seg_limit_sec,
            )
...
def split_segments(
    ...
    def _inner_split_fn(
        ...
        if split_result is None:
            ...
            # Build local probs from the audio chunk directly via load_probs
            local_probs, _ = load_probs(audio_chunk)
        ...
        _inner_split_fn(
            ...
            time_offset_s=right_time_offset_s + (HOP_STEP_MS / 1000),
        )
