...
class SpeechAnalyzer:
    def __init__(
        ...
        min_duration_ms: int
        | None = None,  # minimum raw segment duration in milliseconds
        ...
        self.frame_duration_ms = int(
            round((self.window_size_samples / self.sr) * 1000)
        )  # 32 ms for both 8k/16k
    ...
    def extract_segments(
        ...
        for idx, s in enumerate(segments):
            ...
            seg = SpeechSegment(
                ...
                stats=SegmentStats(
                    avg_prob=round(float(seg_probs.mean()), 3)
                    if len(seg_probs) > 0
                    else 0.0,
                    min_prob=round(float(seg_probs.min()), 3)
                    if len(seg_probs) > 0
                    else 0.0,
                    max_prob=round(float(seg_probs.max()), 3)
                    if len(seg_probs) > 0
                    else 0.0,
                    std_prob=round(float(seg_probs.std()), 3)
                    if len(seg_probs) > 0
                    else 0.0,
                    pct_above_threshold=round(
                        sum(p > self.threshold for p in seg_probs)
                        / len(seg_probs)
                        * 100,
                        1,
                    )
    ...
    def _segment_passes_filters(self, seg: SpeechSegment) -> bool:
        ...
        if self.min_std_prob is not None and seg.stats["std_prob"] < self.min_std_prob:
            return False
        if (
            self.min_pct_threshold is not None
            and seg.stats["pct_above_threshold"] < self.min_pct_threshold
        ):
            return False
        return True
    ...
    def extract_raw_segments(self, prob_array: np.ndarray) -> List[SpeechSegment]:
        ...
        for group_key, group in itertools.groupby(
            ...
            while (
                expand_start > 0 and prob_array[expand_start - 1] > self.neg_threshold
            ):
                expand_start -= 1
            ...
            while (
                expand_end < max_frames and prob_array[expand_end] > self.neg_threshold
            ):
                expand_end += 1
            ...
            raw_seg = SpeechSegment(
                ...
                stats=SegmentStats(
                    ...
                    pct_above_threshold=round(
                        float(np.sum(raw_probs > self.threshold))
                        / len(raw_probs)
                        * 100,
                        1,
                    )
            ...
            raw_seg.stats["pct_above_raw"] = (
                round(
                    float(np.sum(raw_probs > self.raw_threshold))
                    / len(raw_probs)
                    * 100,
                    1,
                )
            ...
            raw_seg.stats["pct_above_neg"] = (
                round(
                    float(np.sum(raw_probs > self.neg_threshold))
                    / len(raw_probs)
                    * 100,
                    1,
                )
    ...
    def plot_insights(
        ...
        if energies is not None:
            ...
            def set_dynamic_xticks(ax, total_seconds: float) -> None:
                ...
                for t in ticks:
                    if t >= 3600:
                        labels.append(
                            f"{int(t // 3600)}:{int((t % 3600) // 60):02d}:{int(t % 60):02d}"
                        )
                    elif t >= 60:
            ...
            overlay_path = (
                Path(out_dir) / f"vad_prob_energy_overlay_{Path(audio_path).stem}.png"
            )
        ...
        plt.legend()
        overlay_path = (
            Path(out_dir) / f"vad_overlay_raw_vs_detected_{Path(audio_path).stem}.png"
        )
        plt.savefig(overlay_path, dpi=300, bbox_inches="tight")
        ...
        if segments:
            ...
            plt.axvline(
                float(np.mean(std_values)),
                color="red",
                linestyle="--",
                label=f"Mean = {float(np.mean(std_values)):.3f}",
            )
            ...
            std_hist_path = (
                Path(out_dir)
                / f"vad_std_probability_histogram_{Path(audio_path).stem}.png"
            )
        ...
        plt.suptitle(
            f"Silero VAD Full Analysis – {Path(audio_path).name}",
            fontsize=18,
            fontweight="bold",
        )
        plt.subplot(2, 1, 1)
        ...
        plt.grid(True, alpha=0.3)
        hist_path = (
            Path(out_dir) / f"vad_probability_histogram_{Path(audio_path).stem}.png"
        )
        ...
        if segments:
            ...
            colors = [
                "green" if p > 0.8 else "orange" if p > 0.6 else "red"
                for p in avg_probs
            ]
            bars = plt.barh(indices, avg_probs, color=colors, alpha=0.8)
            plt.yticks(
                indices,
                [f"Seg {seg.num} ({d:.1f}s)" for seg, d in zip(segments, durations)],
            )
            ...
            conf_path = (
                Path(out_dir) / f"vad_segment_confidence_{Path(audio_path).stem}.png"
            )
        ...
        if segments:
            ...
            fig.suptitle(
                f"Per-Segment Probability Timeline – {Path(audio_path).name}",
                fontsize=16,
            )
            ...
            for idx, seg in enumerate(segments):
                ...
                ax.set_ylim(0, 1.05)
                ax.set_title(
                    f"Seg {seg.num} ({seg.duration}ms | avg={seg.stats['avg_prob']:.3f})"
                )
            ...
            per_seg_path = (
                Path(out_dir)
                / f"vad_per_segment_probability_{Path(audio_path).stem}.png"
            )
    ...
    def _save_segments_individually(
        ...
        *,
        ...
        if sr != self.sr:
            raise ValueError(
                f"Audio sampling rate {sr} does not match analyzer's {self.sr}"
            )
        ...
        for idx, seg in enumerate(segments, start=1):
            ...
            meta.update(
                {
                    ...
                    "segment_frame_count": int(
                        round(seg.duration / self.frame_duration_ms)
                    ),
                }
            )
            ...
            plt.ylim(0, 1.05)
            plt.title(
                f"{title_prefix}Segment {seg.num} Probability Timeline\n"
        ...
        if apply_filters:
            print(
                f"Saved {saved_count}/{total} filtered {title_prefix.lower()}segments → {base_dir}"
            )
        else:
    ...
    def get_metrics(
        ...
        speech_percent = (
            total_speech_sec / total_duration_sec * 100 if total_duration_sec else 0
        )

        raw_durations = [s.duration / 1000 for s in raw_segments]
        raw_total_speech_sec = sum(raw_durations)
        raw_speech_percent = (
            raw_total_speech_sec / total_duration_sec * 100 if total_duration_sec else 0
        )
        ...
        avg_prob_speech = (
            float(prob_array[speech_mask].mean()) if speech_mask.any() else 0.0
        )
        avg_prob_silence = (
            float(prob_array[~speech_mask].mean()) if (~speech_mask).any() else 0.0
        )
        ...
        metrics = {
            ...
            "raw_avg_segment_sec": round(float(np.mean(raw_durations)), 3)
            if raw_durations
            else 0,
        ...
        if energies.size > 0:
            metrics.update(
                {
                    "energy_min": round(float(np.min(energies)), 4)
                    if energies.size
                    else 0,
                    "energy_max": round(float(np.max(energies)), 4)
                    if energies.size
                    else 0,
    ...
    def run_threshold_sweep(
        ...
        for t in thresholds:
            ...
            metrics = analyzer.get_metrics(
                probs, segments, raw_segments, len(probs), total_sec
            )
    ...
    def save_threshold_sweep(
        self, sweep_results: List[Dict], out_dir: str | Path, audio_path: str | Path
    ) -> None:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Silero VAD Full Analyzer – Beautiful insights + JSON"
    )
    parser.add_argument(
        "audio",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Minimum raw segment duration in ms (default: 200)",
    )
    parser.add_argument(
        "--min-std-prob",
        type=float,
        default=0.0,
        help="Minimum std(prob) in raw region (default: 0.0)",
    )
    parser.add_argument(
        "--min-pct-threshold",
        type=float,
        default=10.0,
        help="Min %% windows > threshold in raw region (default: 10.0)",
    )
    ...
    analyzer.plot_insights(
        probs, segments, raw_segments, num_frames, args.audio, args.output_dir
    )
    ...
    analyzer.save_raw_json(
        raw_segments, args.output_dir, args.audio, extra_info=extra_info
    )
