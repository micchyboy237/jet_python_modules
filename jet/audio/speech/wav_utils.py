...
def _validate_audio_data(audio_data: np.ndarray | bytes) -> bytes:
    sample_width = np.dtype(DTYPE).itemsize
    frame_size = CHANNELS * sample_width

    # New helper: convert float audio to target integer dtype PCM
    def _float_to_pcm(arr: np.ndarray, target_dtype: np.dtype = DTYPE) -> np.ndarray:
        if np.issubdtype(arr.dtype, np.floating):
            # Assume [-1,1] range (standard after normalization)
            # But protect against unnormalized large values
            max_abs = np.max(np.abs(arr))
            if max_abs > 1.0001:  # looks like raw / unnormalized
    ...
    if isinstance(audio_data, np.ndarray):
        # Auto-convert float → int if needed
        if np.issubdtype(audio_data.dtype, np.floating):
            audio_data = _float_to_pcm(audio_data)
        ...
        if audio_data.ndim == 1:
            if CHANNELS == 1:
                ...
                audio_data = np.tile(audio_data[:, np.newaxis], (1, CHANNELS))
        if audio_data.ndim != 2 or audio_data.shape[1] != CHANNELS:
            raise ValueError(
                f"Array must have shape (frames, {CHANNELS}) or (frames,) for mono"
            )
...
def save_wav_file(filename, audio_data: np.ndarray | bytes) -> str:
    filename = Path(filename)
    filename.parent.mkdir(parents=True, exist_ok=True)
    data_bytes = _validate_audio_data(audio_data)  # now more tolerant
...
def get_wav_bytes(audio_data: np.ndarray | bytes) -> bytes:
    """
...
def get_wav_fileobj(audio_data: np.ndarray | bytes) -> BinaryIO:
    """
