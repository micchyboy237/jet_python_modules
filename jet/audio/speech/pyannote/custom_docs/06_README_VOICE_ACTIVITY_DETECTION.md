...
### 1. What the Source File Does (Plain English)

`voice_activity_detection.py` answers a simpler question than diarization: **"Is anyone speaking right now?"** — not _who_, just _yes or no_. Given an audio file, it returns a timeline of **speech vs. silence** regions.

### 2. Two Pipelines in the File

| Class                          | Purpose                                                                                               |
| ------------------------------ | ----------------------------------------------------------------------------------------------------- |
| `OracleVoiceActivityDetection` | Cheats — reads the ground-truth annotation directly. Used only for evaluation baselines.              |
| `VoiceActivityDetection`       | The real pipeline — runs a segmentation model, then binarizes soft scores into on/off speech regions. |
...
│  Sliding window → soft scores        │
│  np.max over speaker axis            │
│  → SlidingWindowFeature (prob/frame) │
└───────────────────┬──────────────────┘
                    │
                    ▼
┌──────────────────────────────────────┐
...
│  min_duration_on / off filters       │
│  → Annotation (speech regions)       │
└──────────────────────────────────────┘
```
...
- `Binarize` class
- Powerset vs. non-powerset segmentation models
- The `hook` callback system
- Training cache (`CACHED_SEGMENTATION`)
- `fscore` vs. DER optimization

### 5. Feature Map Sections

- `OracleVoiceActivityDetection`
- `VoiceActivityDetection.__init__`
- `default_parameters`
- `initialize` / `_binarize`
- `apply` (with hook & cache logic)
- `get_metric` / `get_direction`

### 6. Files to Create (4 examples)

1. `example_vad_basic.py` — simplest usage
2. `example_vad_advanced.py` — hooks, tuning, fscore
3. `example_vad_oracle.py` — oracle pipeline + evaluation
4. `example_vad_evaluation.py` — DER/F-score benchmarking
...
This file answers the simplest audio question: _"Is anyone speaking right now?"_

Unlike speaker diarization (which asks _"who_ is speaking?"), **Voice Activity Detection (VAD)** only produces a binary timeline: **SPEECH** or **SILENCE**. The output is a list of time intervals where speech was detected — nothing more.

Two pipelines live in this file:

| Pipeline                       | What it does                                                                                                                                                             |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `OracleVoiceActivityDetection` | Reads the ground-truth answer directly — used only for evaluation baselines. It "cheats" by looking at the reference annotation.                                         |
| `VoiceActivityDetection`       | The real pipeline — runs a neural segmentation model, reduces its output to a single speech-probability curve, then applies thresholds to produce on/off speech regions. |

---

### The 2-Stage Pipeline (How It Works)
```
...
│ → soft probability per frame │
│ np.max() collapses speaker axis │
│ Output: SlidingWindowFeature │
│ (frames × 1 speech prob) │
└──────────────────┬──────────────────┘
│
▼
┌─────────────────────────────────────┐
│ Stage 2: BINARIZE │
│ onset → cross this to START speech│
│ offset → drop below this to STOP │
│ min_duration_on → drop short blips│
│ min_duration_off → fill short gaps │
│ Output: Annotation (SPEECH regions)│
└─────────────────────────────────────┘
...
│
├── OracleVoiceActivityDetection (Pipeline)
│ └── apply() → Returns ground-truth speech regions from file["annotation"]
│
└── VoiceActivityDetection (Pipeline)
├── **init**() → Load segmentation model, configure Inference, set hyper-params
├── default_parameters() → Recommended onset/offset/duration values per model version
├── classes() → Returns ["SPEECH"] — only one label class
├── initialize() → Build Binarize object from current hyper-parameters
├── apply() → Run segmentation → binarize → return Annotation
├── get_metric() → Return DetectionErrorRate or F-measure metric
└── get_direction() → "minimize" (DER) or "maximize" (F-score)
...
|---|---|
| **Voice Activity Detection (VAD)** | Detecting *when* speech happens — ignores *who* is speaking |
...
| **onset** | The probability threshold you must *cross going up* to trigger "speech started" |
| **offset** | The probability threshold you must *drop below* to trigger "speech ended" — usually lower than onset to avoid rapid toggling |
