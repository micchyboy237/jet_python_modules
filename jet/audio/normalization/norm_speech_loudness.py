...
LoudnessPreset = Literal[
    "very_quiet", "quiet", "standard", "loud", "very_loud", "brickwall"
]
...
def normalize_audio_for_vad(
    ...
    if isinstance(max_peak_db, str):
        ...
        if preset_name not in LOUDNESS_PRESETS:
            ...
            raise ValueError(
                f"Invalid preset name: '{max_peak_db}'. Must be one of: {valid_names}"
            )
    ...
    info = {
        ...
        "original_peak": round(original_peak, 4),
        "original_peak_db": round(20 * np.log10(original_peak), 2)
        if original_peak > 0
        else -np.inf,
        ...
        "max_peak_db": round(effective_max_peak_db, 2),
        "preset": preset_name,
        "preset_description": LOUDNESS_PRESETS[preset_name]["description"]
        if preset_name
        else None,
        "skipped_reason": None,
        "sr": sr,
