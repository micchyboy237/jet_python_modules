...
class SearchResult(TypedDict, total=False):
    """
    Structured type for Firecrawl search results.
    """
    url: str
    title: str
    description: str
...
def search_web(
    ...
    This function queries the Firecrawl Search API and returns structured results.
    Optionally, it can scrape each result and return page content.

    Args:
        query: Search query string
        limit: Maximum number of results to return (default: 10)
        scrape: Whether to scrape the result pages (default: False)
        formats: List of content formats to retrieve when scraping.
    ...
    Raises:
        ...
        RuntimeError: If search request fails or response is malformed
    """
    # Validate inputs
    ...
    logger.info(f"Searching web with query: '{query}', limit: {limit}")
    ...
    if scrape:
        search_payload["scrapeOptions"] = {"formats": formats}
    ...
    try:
        with httpx.Client(timeout=timeout) as client:
            response = client.post(
                ...
                headers=headers,
                content=orjson.dumps(search_payload),
            )
            response.raise_for_status()

            search_data = orjson.loads(response.content)
            if not search_data.get("success"):
                raise RuntimeError(f"Search failed: {search_data}")

            results: List[SearchResult] = search_data.get("data", [])
