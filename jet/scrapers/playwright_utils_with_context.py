...
def setup_sync_browser_session(*, headless: bool = False) -> SyncBrowserContext:
    """Create a synchronous Playwright persistent context with dynamic config + stealth.

    Returns a SyncBrowserContext so callers can pass it directly to
    scrape_urls_sync(context=...) and reuse it across multiple calls.
    The caller is responsible for calling context.close() when done.
    """
...
async def setup_async_browser_session(*, headless: bool = False) -> AsyncBrowserContext:
    """Create an asynchronous Playwright persistent context with dynamic config + stealth.

    Returns an AsyncBrowserContext so callers can pass it directly to
    scrape_urls(context=...) and reuse it across multiple calls.
    The caller is responsible for calling await context.close() when done.
    """
    config = get_browser_config()
    effective_headless = headless
    if config.source == "system_chrome" and headless:
...
async def scrape_urls(
    ...
    *,
    ...
    """Asynchronously scrape a list of URLs using Playwright, yielding ScrapeResult for each URL.

    Args:
        urls: List of URLs to scrape.
        context: Optional pre-created AsyncBrowserContext. If None, an ephemeral
                 context is created and destroyed after scraping completes.
                 Use setup_async_browser_session() to create a fully configured
                 context with stealth and validated anti-detection headers.
        ...
        headless: Whether to run browser in headless mode. Only used when context is None.
    ...
    async def sem_fetch_and_yield(
        url: str, ctx: AsyncBrowserContext, pbar=None
    ) -> List[ScrapeResult]:
        results = []
        results.append(
    ...
    try:
        ...
        if owns_context:
            try:
                ...
                logger.debug(f"Failed to close ephemeral context: {e}")
            if browser:
                try:
                    await asyncio.wait_for(browser.close(), timeout=5.0)
                except Exception as e:
                    logger.debug(f"Failed to close ephemeral browser: {e}")
            if pw:
                try:
                    await pw.stop()
                except Exception as e:
                    logger.debug(f"Failed to stop ephemeral playwright: {e}")
...
def scrape_urls_sync(
    ...
    *,
    ...
    Args:
        urls: List of URLs to scrape.
        context: Optional pre-created SyncBrowserContext. If None, an ephemeral
                 async context is created internally (legacy behavior).
                 Use setup_sync_browser_session() to create a fully configured
                 context with stealth and validated anti-detection headers.
        num_parallel: Number of parallel browser tasks.
        limit: Maximum number of successful scrapes to return (None for no limit).
        show_progress: Whether to show a progress bar.
    ...
    if context is not None:
        # When a sync context is provided, use Playwright's sync API directly.
        # This avoids creating nested event loops which causes RuntimeError.
        # Pages are opened/closed sequentially; concurrency is handled by the
        # persistent context's internal browser process.
        from tqdm import tqdm as sync_tqdm

        completed_count = 0
        url_iter = urls
        if show_progress:
            url_iter = sync_tqdm(urls, desc=f"Scraping URLs (sync)", unit="url")

        for url in url_iter:
            # Check cache first (pure sync, no browser needed)
            if use_cache:
                cache_key = f"html:{url}"
                cached = cache.get(cache_key)
                if cached:
                    screenshot = None
                    if with_screenshot and "screenshot" in cached:
                        try:
                            screenshot = base64.b64decode(cached["screenshot"])
                        except Exception as e:
                            logger.error(
                                f"Failed to decode cached screenshot for {url}: {e}"
                            )
                    yield {
                        "url": url,
                        "status": "completed",
            ...
            # Scrape using the sync context directly
            page = None
            try:
                page = context.new_page()
                logger.debug(f"Navigating to {url} (sync context)")
                page.goto(url, timeout=timeout, wait_until="domcontentloaded")

                if wait_for_js:
                    page.wait_for_timeout(3500)

                # Note: scroll_to_bottom is async-only; for sync context we do
                # a simplified single-pass scroll. Full scroll strategies require
                # the async path.
                if scroll_strategy != "none":
                    page.evaluate(
                        "window.scrollTo(0, document.documentElement.scrollHeight)"
                ...
                if use_cache:
                    cache_data = {"content": html_content}
                    if screenshot_bytes:
                        cache_data["screenshot"] = base64.b64encode(
                            screenshot_bytes
                        ).decode("utf-8")
                    cache.set(cache_key, cache_data, ttl=3600)
                ...
                logger.error(f"Error scraping {url} (sync): {str(e)}")
                yield {
                    "url": url,
                    "status": "failed_error",
                    "html": None,
                ...
                if page:
                    try:
                        ...
                        logger.debug(f"Failed to close sync page for {url}: {e}")
        return

    # Legacy path: no context provided, use async implementation
