...
def extract_nodes(
    element, depth: int = 0, parent_id: Optional[str] = None
) -> List[BaseNode]:
    nodes = []
    ...
    id = (
        element_id
        if element_id and re.match(valid_id_pattern, element_id)
        else f"node_{adjusted_depth}_{len(nodes)}"
    )
    class_names = [
        name
        for name in (element_class.split() if element_class else [])
