...
def log_prompt_details(system_prompt: str, instruction: str, model_path: LLMModelType) -> None:
    """Logs system prompt, tokenized system prompt, and instruction for debugging."""
    logger.gray("System:")
...
def format_chat_messages(system_prompt: str, instruction: str) -> List[ChatMessage]:
...
def instruction_response(
    ...
    top_p: float = 0.9,
    system_prompt: str = "Follow the instruction provided accurately and concisely."
) -> InstructionResult:
    """Generates a response to the input instruction."""
    try:
        if not instruction.strip():
            raise PromptFormattingError("Instruction cannot be empty.")
        validate_method(method)
        model_components = load_model_components(model_path)
        log_prompt_details(system_prompt, instruction, model_path)
        messages = format_chat_messages(system_prompt, instruction)
