...
def generate_relations_stream(
    ...
    """Generates relations using stream_generate method."""
    relations = ""
    token_ids = []
...
def generate_relations_step(
    ...
    """Generates relations using generate_step method."""
    relations = ""
    token_ids = []
...
def relation_extraction(
    ...
    top_p: float = 0.9,
    system_prompt: str = "Perform relation extraction on the following text. Identify relationships between entities (e.g., Person, Organization, Location) and list them in the format: Entity1: <entity1>, Relation: <relation>, Entity2: <entity2>. If no relations are found, state 'No relations found.'"
) -> RelationExtractionResult:
    """Performs relation extraction on the input text."""
    try:
        if not text.strip():
        ...
        if method == "stream_generate":
            relations, token_ids = generate_relations_stream(
                model_components, formatted_prompt, max_tokens,
                logits_processors, sampler, stop_tokens
            )
        else:
            relations, token_ids = generate_relations_step(
                model_components, formatted_prompt, max_tokens,
                logits_processors, sampler, stop_tokens
            )
