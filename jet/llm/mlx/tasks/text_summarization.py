...
def create_system_prompt() -> str:
    """Creates a formatted system prompt for text summarization."""
    return (
        "You are an expert summarizer. Summarize the given input text in a concise manner, "
        "capturing the main points in 1-2 sentences. Provide only the summary without any additional explanation."
    )
...
def validate_response(response: str) -> None:
    ...
    if not response.strip():
        raise InvalidOutputError("Summary is empty.")
...
def text_summarization(
    ...
    Returns:
        SummarizationResult: Dictionary containing the summary and metadata.
    """
    try:
