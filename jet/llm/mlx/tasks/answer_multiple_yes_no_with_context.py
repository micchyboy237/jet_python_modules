...
if __name__ == "__main__":
    ...
    results = answer_multiple_yes_no_with_context(questions_contexts, model_path)
...
class TestAnswerMultipleYesNo:
    ...
    def test_no_answer(self):
        ...
        result = answer_multiple_yes_no_with_context(questions_contexts, self.model_path)[0]
        expected = {"answer": "No", "confidence": float, "is_valid": True, "error": None}
    ...
    def test_yes_answer(self):
        ...
        result = answer_multiple_yes_no_with_context(questions_contexts, self.model_path)[0]
        expected = {"answer": "Yes", "confidence": float, "is_valid": True, "error": None}
        assert result["answer"] == expected["answer"], f"Expected answer {expected['answer']}, got {result['answer']}"
        assert isinstance(result["confidence"], float), "Confidence should be a float"
        assert result["is_valid"] == expected["is_valid"], f"Expected is_valid {expected['is_valid']}, got {result['is_valid']}"
        assert result["error"] == expected["error"], f"Expected error {expected['error']}, got {result['error']}"
    ...
    def test_invalid_input_empty_question(self):
        ...
        with pytest.raises(InvalidInputError):
            answer_multiple_yes_no_with_context(questions_contexts, self.model_path)
