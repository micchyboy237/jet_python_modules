...
class ParaphraseResult(TypedDict):
    paraphrase: str
    is_valid: bool
    method: str
    error: Optional[str]
...
def create_system_prompt() -> str:
    """Creates a formatted system prompt for paraphrasing."""
    return (
        "You are a language expert. Paraphrase the given input text in English, "
        "keeping the meaning intact but using different words and sentence structure. "
        "Provide only the paraphrased text without any additional explanation."
    )
...
def validate_response(response: str) -> None:
    ...
    if not response.strip():
        raise InvalidOutputError("Paraphrase is empty.")
...
def paraphrase(
    ...
    """
    Generates a paraphrase of the input text in English.

    Args:
        input_text (str): The input text to paraphrase.
    ...
    Returns:
        ParaphraseResult: Dictionary containing paraphrase and metadata.
    """
    try:
