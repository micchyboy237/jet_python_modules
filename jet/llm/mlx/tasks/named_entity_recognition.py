...
def generate_entities_stream(
    ...
    """Generates entities using stream_generate method."""
    entities = ""
    token_ids = []
    for output in stream_generate(
        ...
        entities = model_components.tokenizer.decode(token_ids)
    return entities, token_ids


def generate_entities_step(
    model_components: ModelComponents,
    formatted_prompt: str,
    max_tokens: int,
    logits_processors: list,
    ...
    """Generates entities using generate_step method."""
    entities = ""
    token_ids = []
...
def named_entity_recognition(
    ...
    top_p: float = 0.9,
    system_prompt: str = "Perform Named Entity Recognition (NER) on the following text. Identify entities such as Person, Organization, and Location, and list them in the format: Entity: <entity_name>, Type: <entity_type>. If no entities are found, state 'No entities found.'"
) -> NERResult:
    ...
    try:
        ...
        if method == "stream_generate":
            entities, token_ids = generate_entities_stream(
                model_components, formatted_prompt, max_tokens,
                logits_processors, sampler, stop_tokens
            )
        else:
            entities, token_ids = generate_entities_step(
                model_components, formatted_prompt, max_tokens,
                logits_processors, sampler, stop_tokens
            )
        return NERResult(
