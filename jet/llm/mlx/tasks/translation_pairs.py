...
def create_system_prompt(target_language: str) -> str:
    """Creates a formatted system prompt for translation."""
    return (
        f"You are a language expert. Translate the given input text into {target_language}. "
        f"Provide only the translated text without any additional explanation."
    )
...
def validate_response(response: str) -> None:
    ...
    if not response.strip():
        raise InvalidOutputError("Translation is empty.")
...
def translation_pairs(
    ...
    """
    Generates a translation of the input text into the target language.

    Args:
        input_text (str): The input text to translate.
        target_language (str): The target language for translation (e.g., "Spanish").
    ...
    Returns:
        TranslationResult: Dictionary containing translation and metadata.
    """
    try:
        ...
        system_prompt = create_system_prompt(target_language)
