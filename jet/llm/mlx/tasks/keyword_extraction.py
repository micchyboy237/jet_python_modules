...
def generate_keywords_stream(
    ...
    """Generates keywords using stream_generate method."""
    keywords = ""
    token_ids = []
    for output in stream_generate(
        ...
        keywords = model_components.tokenizer.decode(token_ids)
    return keywords, token_ids


def generate_keywords_step(
    model_components: ModelComponents,
    formatted_prompt: str,
    max_tokens: int,
    logits_processors: list,
    ...
    """Generates keywords using generate_step method."""
    keywords = ""
    token_ids = []
...
def keyword_extraction(
    ...
    top_p: float = 0.9,
    system_prompt: str = "Extract the most important keywords or phrases from the following text. List them as a comma-separated string. If no keywords are identified, state 'No keywords found.'"
) -> KeywordExtractionResult:
    """Performs keyword extraction on the input text."""
    try:
        if not text.strip():
        ...
        if method == "stream_generate":
            keywords, token_ids = generate_keywords_stream(
                model_components, formatted_prompt, max_tokens,
                logits_processors, sampler, stop_tokens
            )
        else:
            keywords, token_ids = generate_keywords_step(
                model_components, formatted_prompt, max_tokens,
                logits_processors, sampler, stop_tokens
            )
