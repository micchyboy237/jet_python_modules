...
def create_system_prompt(sentiments: List[str]) -> str:
    """Creates a formatted system prompt with the given sentiment options."""
...
def encode_sentiments(tokenizer: TokenizerWrapper, sentiments: List[str]) -> Dict[str, List[int]]:
    """Encodes each sentiment into tokens and logs the results."""
    sentiment_token_map = {}
    for sentiment in sentiments:
        tokens = tokenizer.encode(sentiment, add_special_tokens=False)
        sentiment_token_map[sentiment] = tokens
        logger.log(f"Tokens for '{sentiment}':",
                   tokens, colors=["GRAY", "ORANGE"])
    return sentiment_token_map


def setup_generation_parameters(
    tokenizer: TokenizerWrapper,
    sentiment_token_map: Dict[str, List[int]],
    temperature: float,
    top_p: float
) -> tuple:
    """Sets up logit bias, logits processors, sampler, and stop tokens for generation."""
    logit_bias = {tokens[0]: 0.0 for sentiment,
                  tokens in sentiment_token_map.items() if tokens}
...
def generate_sentiment_stream(
    ...
    """Generates a sentiment using stream_generate method."""
    sentiment = ""
    token_id = -1
    generated_tokens = []
    for output in stream_generate(
        ...
        sentiment = model_components.tokenizer.decode(generated_tokens)
        if sentiment in sentiments:
            break
    return sentiment, token_id, generated_tokens


def generate_sentiment_step(
    model_components: ModelComponents,
    formatted_prompt: str,
    max_tokens: int,
    logits_processors: list,
    ...
    """Generates a sentiment using generate_step method."""
    sentiment = ""
    token_id = -1
    generated_tokens = []
...
def validate_sentiment(sentiment: str, sentiments: List[str]) -> None:
    """Validates that the generated sentiment is one of the provided options."""
    if sentiment not in sentiments:
        raise InvalidOutputError(
            f"Output '{sentiment}' is not one of the provided sentiments.")
...
def sentiment_analysis(
    ...
    """Performs multi-class sentiment analysis on the input text."""
    try:
        validate_method(method)
        model_components = load_model_components(model_path)
        system_prompt = create_system_prompt(sentiments)
        ...
        sentiment_token_map = encode_sentiments(
            model_components.tokenizer, sentiments)
        logits_processors, sampler, stop_tokens = setup_generation_parameters(
            model_components.tokenizer, sentiment_token_map, temperature, top_p
        )
        if method == "stream_generate":
            sentiment, token_id, _ = generate_sentiment_stream(
                model_components, formatted_prompt, max_tokens,
                logits_processors, sampler, stop_tokens, sentiments
            )
        else:
            sentiment, token_id, _ = generate_sentiment_step(
                model_components, formatted_prompt, max_tokens,
                logits_processors, sampler, stop_tokens, sentiments
            )
        validate_sentiment(sentiment, sentiments)
