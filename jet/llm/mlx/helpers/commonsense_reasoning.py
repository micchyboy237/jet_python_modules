...
class CommonsenseReasoningResult(TypedDict):
    answer: str
    token_ids: List[int]
    is_valid: bool
    method: str
...
def log_prompt_details(system_prompt: str, question: str, model_path: ModelType) -> None:
    """Logs system prompt, tokenized system prompt, and question for debugging."""
    logger.gray("System:")
    logger.debug(system_prompt)
    logger.gray("Tokenized System:")
    logger.debug(tokenize_strings(system_prompt, model_path))
    logger.gray("Question:")
...
def generate_answer_stream(
    ...
    for output in stream_generate(
        ...
        answer = model_components.tokenizer.decode(token_ids)
...
def commonsense_reasoning(
    ...
    top_p: float = 0.9,
    system_prompt: str = "Answer the following question using commonsense reasoning. Provide a concise and logical response based on everyday knowledge."
) -> CommonsenseReasoningResult:
    """Generates an answer to a commonsense reasoning question."""
    try:
        ...
        if method == "stream_generate":
            answer, token_ids = generate_answer_stream(
                model_components, formatted_prompt, max_tokens,
                logits_processors, sampler, stop_tokens
            )
        else:
            answer, token_ids = generate_answer_step(
                model_components, formatted_prompt, max_tokens,
                logits_processors, sampler, stop_tokens
            )
        return CommonsenseReasoningResult(
            answer=answer.strip(),
            token_ids=token_ids,
            is_valid=True,
            method=method,
