...
class ConditionalGenerationResult(TypedDict):
    output: str
    token_ids: List[int]
    is_valid: bool
    method: str
...
def create_system_prompt(conditions: Dict[str, str]) -> str:
    """Creates a formatted system prompt incorporating the provided conditions."""
    condition_str = "\n".join([f"{key}: {value}" for key, value in conditions.items()])
    return f"Generate text based on the following prompt, adhering to the specified conditions.\nConditions:\n{condition_str}"
