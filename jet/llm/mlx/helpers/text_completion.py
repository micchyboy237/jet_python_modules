...
def log_prompt_details(system_prompt: str, prompt: str, model_path: ModelType) -> None:
...
def generate_completion_stream(
    ...
    """Generates a completion using stream_generate method."""
    completion = ""
    token_ids = []
    for output in stream_generate(
        ...
        completion = model_components.tokenizer.decode(token_ids)
    return completion, token_ids

def generate_completion_step(
    model_components: ModelComponents,
    formatted_prompt: str,
    max_tokens: int,
    logits_processors: list,
    ...
    """Generates a completion using generate_step method."""
    completion = ""
    token_ids = []
...
def text_completion(
    ...
    top_p: float = 0.9,
    system_prompt: str = "Continue the following text naturally and coherently."
) -> CompletionResult:
    """Generates a completion for the input prompt."""
    try:
        if not prompt.strip():
        ...
        if method == "stream_generate":
            completion, token_ids = generate_completion_stream(
                model_components, formatted_prompt, max_tokens,
                logits_processors, sampler, stop_tokens
            )
        else:
            completion, token_ids = generate_completion_step(
                model_components, formatted_prompt, max_tokens,
                logits_processors, sampler, stop_tokens
            )
