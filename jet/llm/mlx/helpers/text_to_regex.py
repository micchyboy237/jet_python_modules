...
class TextToRegexResult(TypedDict):
    regex: str
    is_valid: bool
    method: str
    error: Optional[str]
...
def create_system_prompt() -> str:
    """Creates a formatted system prompt for text-to-regex conversion."""
    return (
        "You are a regular expression expert. Convert the given natural language description into a valid Python-compatible regular expression. "
        "Provide only the regex pattern without any additional explanation or delimiters (e.g., no `/` or `r''`)."
    )
...
def validate_response(response: str) -> None:
    """Validates that the response is a non-empty regex pattern."""
    if not response.strip():
        raise InvalidOutputError("Regex pattern is empty.")
    # Basic validation: ensure response contains at least some regex-like characters
    if not any(char in response for char in r".*+?[]^$|"):
        raise InvalidOutputError("Generated response does not resemble a valid regex pattern.")
...
def text_to_regex(
    ...
    """
    Converts a natural language description into a Python-compatible regular expression.

    Args:
        input_text (str): The natural language description of the regex pattern.
    ...
    Returns:
        TextToRegexResult: Dictionary containing the regex pattern and metadata.
    """
    try:
