...
class TestGetWordCountsLemmatized:
    ...
    def test_single_string_with_percent_threshold(self):
        ...
        # Total words: 6 (cat, run, jump, quickly, cat, cat)
        # 33.33% threshold => min count = ceil(6 * 33.33 / 100) = 2
        expected = {'cat': 3}
    ...
    def test_single_string_with_score(self):
        ...
        #             jump (count=1, len=4): 1 * (1 + log(4)) ≈ 2.386
        #             quickly (count=1, len=7): 1 * (1 + log(7)) ≈ 2.946
        # Max raw score: 4.197 (cat)
        # Normalized: cat: (4.197 / 4.197) * 100 = 100
        #             quickly: (2.946 / 4.197) * 100 ≈ 70.19
        #             jump: (2.386 / 4.197) * 100 ≈ 56.85
        #             run: (2.099 / 4.197) * 100 ≈ 50.01
        expected = {
            'cat': 100.0,
            'quickly': (2.946 / 4.197) * 100,
    ...
    def test_list_of_strings_with_score(self):
        ...
        # First text: cat (count=2, len=3): 2 * (1 + log(3)) ≈ 4.197
        #             run (count=1, len=3): 1 * (1 + log(3)) ≈ 2.099
        #             jump (count=1, len=4): 1 * (1 + log(4)) ≈ 2.386
        #             quickly (count=1, len=7): 1 * (1 + log(7)) ≈ 2.946
        # Max score: 4.197 (cat)
        # Second text: dog (count=1, len=3): 1 * (1 + log(3)) ≈ 2.099
        #              bark (count=1, len=4): 1 * (1 + log(4)) ≈ 2.386
        #              run (count=1, len=3): 1 * (1 + log(3)) ≈ 2.099
        #              loudly (count=1, len=6): 1 * (1 + log(6)) ≈ 2.792
        #              fast (count=1, len=4): 1 * (1 + log(4)) ≈ 2.386
        # Max score: 2.792 (loudly)
        expected = [
            {
                'cat': 100.0,
                'quickly': (2.946 / 4.197) * 100,
            ...
            {
                'loudly': 100.0,
                'bark': (2.386 / 2.792) * 100,
                'fast': (2.386 / 2.792) * 100,
                'dog': (2.099 / 2.792) * 100,
                'run': (2.099 / 2.792) * 100
            }
        ]
