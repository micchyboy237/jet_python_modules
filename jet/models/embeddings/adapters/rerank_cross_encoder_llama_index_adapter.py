...
class CrossEncoderRerank(BaseNodePostprocessor):
    ...
    def __init__(
        ...
        # Set device to default "cpu" if None is provided
        device = device or "cpu"
        logger.debug(f"Initializing CrossEncoderRerank with device: {device}")
        super().__init__(top_n=top_n, model=model, device=device)
        try:
            self._model = CrossEncoderRegistry.load_model(model, device=device)
            logger.info(
                f"Initialized CrossEncoderRerank with model: {model}, device: {device}, top_n: {top_n}")
    ...
    def _postprocess_nodes(
        ...
        logger.debug(
            f"Starting _postprocess_nodes with top_n: {self.top_n}, device: {self.device}")
        dispatcher.event(
            ReRankStartEvent(
