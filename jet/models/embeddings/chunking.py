...
- Source text indices include both header-inclusive range (`start_idx`, `end_idx`)
  for full semantic-unit reconstruction AND non-overlapping body range
  (`body_start_idx`, `body_end_idx`) for highlighting/deduplication.
...
- Sentence alignment failures raise ValueError instead of silently corrupting indices.
- Optional word-level chunk overlap support for improved RAG retrieval quality.
...
def chunk_headers_by_hierarchy(
    ...
    Args:
        ...
        overlap_tokens: Number of words from end of previous chunk to prepend
                        to next chunk (not counted toward budget). Default 0.
    ...
    Raises:
        ValueError: If sentence splitter output cannot be aligned to source text.
    """
    ...
    section_counter = -1  # Increments to 0 for first header; preamble = -1
    ...
    def _flush_chunk():
        ...
        chunk: ChunkResult = {
            ...
            "metadata": {
                ...
                "body_start_idx": current["_body_abs_start"],
                "body_end_idx": current["_body_abs_end"],
            },
        }
        results.append(chunk)
        current["chunk_index"] += 1

        # Compute word-level overlap for next chunk
        if overlap_tokens > 0 and current["content_parts"]:
            all_words: List[str] = []
            for part in current["content_parts"]:
                all_words.extend(part.split())

            overlap_words = (
                all_words[-overlap_tokens:]
                if len(all_words) >= overlap_tokens
                else all_words
            )
            overlap_str = " ".join(overlap_words)

            if overlap_str:
                current["_overlap_parts"] = [overlap_str]
                current["_overlap_token_count"] = _count_tokens(overlap_str)
            else:
                current["_overlap_parts"] = []
                current["_overlap_token_count"] = 0
        else:
            current["_overlap_parts"] = []
            current["_overlap_token_count"] = 0

        current["content_parts"] = list(current["_overlap_parts"])
        current["token_count"] = current["_overlap_token_count"]

        # Reset anchors to header position for next chunk in same section
        current["abs_start"] = current["_header_abs_start"]
        current["abs_end"] = current["_header_abs_start"]
        current["_body_abs_start"] = 0
        current["_body_abs_end"] = 0
    ...
    for line in lines:
        ...
        if stripped:
            ...
            for sent in sentences:
                ...
                if idx == -1:
                    raise ValueError(
                        f"Sentence alignment failed: {sent_stripped!r} not found "
                        f"in line at offset {search_offset}. Check split_fn output "
                        f"matches source text exactly."
                    )
                ...
                if not current["content_parts"] or (
                    current["_body_abs_start"] == 0
                    and current["_overlap_token_count"] == 0
                ):
                    current["abs_start"] = current["_header_abs_start"]
                    current["_body_abs_start"] = abs_sent_start

                current["content_parts"].append(sent_stripped)
                current["token_count"] += sent_token_count
                current["abs_end"] = abs_sent_end
                current["_body_abs_end"] = abs_sent_end
...
def chunk_docs_by_hierarchy(
    ...
    Args:
        ...
        overlap_tokens: Words to overlap between consecutive chunks.
    ...
    for doc_idx, (markdown_text, doc_id) in enumerate(zip(markdown_texts, doc_ids)):
        doc_chunks = chunk_headers_by_hierarchy(
            markdown_text, chunk_size, tokenizer, split_fn, overlap_tokens
        )
