...
async def main():
    ...
    config_default = get_adaptive_config(strategy="embedding", max_pages=10)

    await test_configuration(
        "Default Settings", config_default, test_url, "async programming patterns"
    )

    # 2. Strict Coverage Requirements
    config_strict = get_adaptive_config(
        strategy="embedding",
        max_pages=20,
    ...
    # 3. Fast Exploration
    config_fast = get_adaptive_config(
        strategy="embedding",
        max_pages=10,
    ...
    await test_configuration(
        "Fast Exploration (Quick Overview)", config_fast, test_url, "async basics"
    )

    # 4. Irrelevance Detection Focus
    config_irrelevance = get_adaptive_config(
        strategy="embedding",
        max_pages=5,
    ...
    config_quality = get_adaptive_config(
        strategy="embedding",
        max_pages=30,
