...
async def build_knowledge_base():
    ...
    async with AsyncWebCrawler(verbose=False) as crawler:
        ...
        await adaptive.digest(
            start_url="https://httpbin.org", query="http methods headers status codes"
        )
...
async def analyze_knowledge_base(kb_path):
    ...
    print(
        f"  - Average per document: {total_content_length / len(documents):,.0f} chars"
    )
...
async def share_knowledge_bases():
    ...
    async with AsyncWebCrawler(verbose=False) as crawler:
        ...
        await crawler_b.digest(
            start_url="https://httpbin.org/anything", query="testing endpoints mocking"
        )
