...
async def main():
    ...
    for test in test_cases:
        ...
        stat_result = await crawl_with_strategy(
            test["url"], test["query"], "statistical"
        )
        ...
        print(
            f"  Sufficient: {'Yes' if stat_result['crawler'].is_sufficient else 'No'}"
        )
        ...
        print("\n📈 Comparison:")
        efficiency_diff = (
            ((stat_result["pages"] - emb_result["pages"]) / stat_result["pages"] * 100)
            if stat_result["pages"] > 0
            else 0
        )
        ...
        if stat_result["elapsed"] < emb_result["elapsed"]:
            print(
                f"Statistical was {emb_result['elapsed'] / stat_result['elapsed']:.1f}x faster"
            )
        else:
            print(
                f"Embedding was {stat_result['elapsed'] / emb_result['elapsed']:.1f}x faster"
            )

        print(
            f"  Confidence difference: {abs(stat_result['confidence'] - emb_result['confidence']) * 100:.0f} percentage points"
        )

        # Recommendation
        print("\n💡 Recommendation:")
        if "specific" in test["name"].lower() or all(
            len(term) > 5 for term in test["query"].split()
        ):
            print(
                "  → Statistical strategy is likely better for this use case (specific terms)"
            )
        elif "conceptual" in test["name"].lower() or "semantic" in test["name"].lower():
            print(
                "  → Embedding strategy is likely better for this use case (semantic understanding)"
            )
        else:
            if emb_result["confidence"] > stat_result["confidence"] + 0.1:
                print(
                    "  → Embedding strategy achieved significantly better understanding"
                )
            elif stat_result["elapsed"] < emb_result["elapsed"] / 2:
                print("  → Statistical strategy is much faster with similar results")
            else:
                print(
                    "  → Both strategies performed similarly; choose based on your priorities"
                )
