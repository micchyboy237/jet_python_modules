...
class APIDocumentationStrategy:
    ...
    def calculate_api_coverage(self, state: CrawlState, query: str) -> dict[str, float]:
        ...
        for doc in state.knowledge_base:
            content = (
                doc.markdown.raw_markdown if hasattr(doc, "markdown") else str(doc)
            )
            ...
            if any(
                re.search(pattern, content, re.IGNORECASE) for pattern in param_patterns
            ):
                docs_with_params += 1
...
class ResearchPaperStrategy:
    ...
    def calculate_academic_relevance(self, content: str, query: str) -> float:
        ...
        # Check for citations
        citation_count = sum(
            len(re.findall(pattern, content)) for pattern in self.citation_patterns
        )
        ...
        for term in query_terms:
            ...
            for keyword in ["abstract", "conclusion", "results"]:
                if keyword in content_lower:
                    section = content_lower[
                        content_lower.find(keyword) : content_lower.find(keyword) + 500
                    ]
                    if term in section:
                        score += 0.2
...
async def demo_custom_strategies():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        config = get_adaptive_config(confidence_threshold=0.8, max_pages=15)
    ...
    class HybridStrategy:
        ...
        def __init__(self):
            ...
            self.weights = {"api": 0.7, "research": 0.3}
        ...
        def score_content(self, content: str, query: str) -> float:
            ...
            research_score = self.research_strategy.calculate_academic_relevance(
                content, query
            )
...
async def demo_performance_optimization():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        state = await adaptive.digest(
            start_url="https://httpbin.org", query="http methods headers"
        )
        ...
        print(
            f"  - Efficiency: {adaptive.confidence / len(state.crawled_urls):.2%} confidence per page"
        )
