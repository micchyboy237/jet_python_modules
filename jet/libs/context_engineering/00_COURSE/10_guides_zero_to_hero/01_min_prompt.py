...
def example_5_consistency():
    ...
    def measure_consistency(prompt: str, n_samples: int = 3) -> List[Dict]:
        responses = []
        for i in range(n_samples):
            response, metadata = llm.generate_response(prompt)
            sub_dir = example_dir / f"sample_{i+1}"
            save_example_artifacts(sub_dir, {
                "title": f"Sample {i+1}",
                "prompt": prompt,
                "response": response,
...
Focus on the feeling of solving a difficult bug."""
    ...
    basic_responses = measure_consistency(basic_prompt, n_samples=3)
    enhanced_responses = measure_consistency(enhanced_prompt, n_samples=3)
