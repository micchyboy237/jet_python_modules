...
class PedagogicalModel:
    ...
    def execute_strategy(self, strategy: Dict[str, Any], student_model: StudentKnowledgeModel,
        ...
        """
        learning_experience = []
        last_assessment_data = None  # Track performance for feedback
        ...
        for tool_step in strategy["tool_sequence"]:
            ...
            if tool_name in self.tools:
                ...
                # Pass content_model only to tools that accept it
                if tool_name in {
                ...
                # Pass concept only to tools that accept it
                if tool_name in {"explanation_tool", "practice_tool", "assessment_tool"}:
                    extra_kwargs["concept"] = tool_params.get("concept") or "example_concept"
                
                # === FIX: Pass performance to feedback_tool ===
                ...
                result = self.tools[tool_name](
                    ...
                    **extra_kwargs,
                    **{k: v for k, v in tool_params.items() if k not in {"concept"}}
                )
                ...
                # === Update last_assessment_data if available ===
