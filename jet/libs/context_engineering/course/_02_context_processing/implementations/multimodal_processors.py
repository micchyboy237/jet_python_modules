...
main_logger.info("=" * 80)
main_logger.info("MULTIMODAL PROCESSORS LAB STARTED")
...
def save_json(data: Any, filepath: str) -> None:
    with open(filepath, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, default=lambda o: str(o) if hasattr(o, "__dict__") else str(o))
...
def example_01_individual_encoders():
    name = "example_01_individual_encoders"
    ex_dir = create_example_dir(name)
    log = get_example_logger(name, ex_dir)

    log.info("Testing individual modality encoders")
    sample_data = create_sample_data(d_model=512)

    results = {}
    for mod_name, (cls, data_key) in [
        ("text", (TextEncoder, Modality.TEXT)),
        ("image", (ImageEncoder, Modality.IMAGE)),
        ("audio", (AudioEncoder, Modality.AUDIO)),
    ]:
        encoder = cls(d_model=512)
        embedding = encoder.encode(sample_data[data_key])
        norm = float(np.linalg.norm(embedding))
        results[mod_name] = {"shape": list(embedding.shape), "norm": norm}
        log.info(f"{mod_name.capitalize()} encoder → shape: {embedding.shape} | norm: {norm:.4f}")

    save_json(results, os.path.join(ex_dir, "individual_encoders.json"))
    log.info("Individual encoder results saved")


def example_02_fusion_strategies():
    name = "example_02_fusion_strategies"
    ex_dir = create_example_dir(name)
    log = get_example_logger(name, ex_dir)

    log.info("Comparing different fusion strategies")
    sample_data = create_sample_data(d_model=512)

    # Encode individually first
    encoders = {
        Modality.TEXT: TextEncoder(d_model=512),
        Modality.IMAGE: ImageEncoder(d_model=512),
        Modality.AUDIO: AudioEncoder(d_model=512),
    }
    individual_embs = [
        encoders[Modality.TEXT].encode(sample_data[Modality.TEXT]),
        encoders[Modality.IMAGE].encode(sample_data[Modality.IMAGE]),
        encoders[Modality.AUDIO].encode(sample_data[Modality.AUDIO]),
    ]

    fusion_results = {}
    for strategy_name, cls in [
        ("attention", AttentionFusion),
        ("concatenation", ConcatenationFusion),
        ("gated", GatedFusion),
    ]:
        fusion = cls(d_model=512)
        fused = fusion.fuse(individual_embs)
        norm = float(np.linalg.norm(fused))
        fusion_results[strategy_name] = {"norm": norm, "shape": list(fused.shape)}
        log.info(f"{strategy_name.capitalize()} fusion → norm: {norm:.4f}")

    save_json(fusion_results, os.path.join(ex_dir, "fusion_comparison.json"))
    log.info("Fusion strategy comparison saved")


def example_03_complete_processor():
    name = "example_03_complete_processor"
    ex_dir = create_example_dir(name)
    log = get_example_logger(name, ex_dir)

    log.info("Full MultimodalProcessor pipeline (attention fusion)")
    ...
    log.info(f"Fused embedding norm : {np.linalg.norm(result['fused_embedding']):.4f}")
    log.info(f"Fusion strategy      : {result['fusion_strategy']}")

    save_json(result, os.path.join(ex_dir, "processor_result.json"))
    log.info("Complete processor result saved")


def example_04_modality_alignment():
    name = "example_04_modality_alignment"
    ex_dir = create_example_dir(name)
    log = get_example_logger(name, ex_dir)

    log.info("Testing cross-modal alignment and similarity matrix")
    sample_data = create_sample_data(d_model=512)

    encoders = {m: cls(d_model=512) for m, cls in [
        (Modality.TEXT, TextEncoder),
        (Modality.IMAGE, ImageEncoder),
        (Modality.AUDIO, AudioEncoder),
    ]}
    raw_embs = {m: encoders[m].encode(sample_data[m]) for m in encoders}
    ...
    log.info("Aligned embedding norms:")
    for m, emb in aligned.items():
        log.info(f"  {m.value:6s} → {np.linalg.norm(emb):.4f}")

    # Save matrix + text visualization
    save_json({
        "similarity_matrix": similarity_matrix.tolist(),
        "modalities": [m.value for m in aligned.keys()]
    }, os.path.join(ex_dir, "similarity_matrix.json"))

    with open(os.path.join(ex_dir, "similarity_report.md"), "w", encoding="utf-8") as f:
        f.write(visualize_modality_similarities(
            similarity_matrix,
            [m.value for m in aligned.keys()]
        ))

    log.info("Alignment & similarity matrix saved")


def example_05_multimodal_rag():
    name = "example_05_multimodal_rag"
    ex_dir = create_example_dir(name)
    log = get_example_logger(name, ex_dir)

    log.info("Multimodal RAG: indexing + retrieval")
    rag = MultimodalRAG(d_model=512)

    # Index 5 documents
    for i in range(5):
        data = create_sample_data(d_model=512)
        ...
        log.info(f"Indexed document doc_{i:02d}")

    # Query
    query_data = create_sample_data(d_model=512)
    ...
    log.info("Top 3 retrieved documents:")
    for r in retrieved:
        log.info(f"  → {r['doc_id']} | similarity: {r['similarity']:.4f}")

    save_json([{
        "doc_id": r["doc_id"],
        "similarity": r["similarity"],
        "metadata": r["document"]["metadata"]
    } for r in retrieved], os.path.join(ex_dir, "retrieval_results.json"))

    log.info("RAG retrieval results saved")


def example_06_benchmark():
    name = "example_06_benchmark"
    ex_dir = create_example_dir(name)
    log = get_example_logger(name, ex_dir)

    log.info("Performance benchmark (10 trials)")
    bench = benchmark_multimodal_processing(
        lambda: MultimodalProcessor(d_model=512, fusion_strategy="attention"),
        num_trials=10
    )

    log.info(f"Mean processing time : {bench['mean_processing_time']*1000:.2f} ms")
    log.info(f"Mean modalities      : {bench['mean_modalities_processed']:.1f}")
    log.info(f"Success rate         : {bench['success_rate']*100:.1f}%")

    save_json(bench, os.path.join(ex_dir, "benchmark_results.json"))
    log.info("Benchmark completed & saved")


# <<< REPLACE THE OLD __main__ BLOCK WITH THIS >>>
if __name__ == "__main__":
    main_logger.info("Starting multimodal processors examples...")

    example_01_individual_encoders()
    example_02_fusion_strategies()
    example_03_complete_processor()
    example_04_modality_alignment()
    example_05_multimodal_rag()
    example_06_benchmark()

    main_logger.info("All multimodal examples completed!")
