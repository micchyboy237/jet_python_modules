...
def demonstrate_basic_assembly():
    ...
    save_file(greedy_result, f"{OUTPUT_DIR}/basic_assembly/greedy_result.json")
    ...
    save_file(optimal_result, f"{OUTPUT_DIR}/basic_assembly/optimal_result.json")
    ...
    save_file(greedy_metrics, f"{OUTPUT_DIR}/basic_assembly/greedy_metrics.json")
    ...
    for metric, value in optimal_metrics.items():
        print(f"{metric}: {value:.3f}")
    save_file(optimal_metrics, f"{OUTPUT_DIR}/basic_assembly/optimal_metrics.json")
...
def demonstrate_assembly_patterns():
    ...
    for key, value in patterns_results.items():
        save_file(value, f"{OUTPUT_DIR}/assembly_patterns/{format_file_path(key)}.json")
...
def demonstrate_optimization_comparison():
    ...
    for i, constraints in enumerate(constraints_scenarios):
        ...
        save_file(constraints, f"{OUTPUT_DIR}/optimization_comparison/constraint_{i + 1}/constraints.json")
        save_file(greedy_result, f"{OUTPUT_DIR}/optimization_comparison/constraint_{i + 1}/greedy_result.json")
        save_file(optimal_result, f"{OUTPUT_DIR}/optimization_comparison/constraint_{i + 1}/optimal_result.json")
        save_file(greedy_metrics, f"{OUTPUT_DIR}/optimization_comparison/constraint_{i + 1}/greedy_metrics.json")
        save_file(optimal_metrics, f"{OUTPUT_DIR}/optimization_comparison/constraint_{i + 1}/optimal_metrics.json")
