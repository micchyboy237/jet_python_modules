...
logger = logging.getLogger("vision-stream-merged-async")
...
async def fetch_remote_image_bytes_async(
    ...
    try:
        ...
        if hasattr(exc, "response") and exc.response is not None:
            error_detail = f" (status={exc.response.status_code}, reason={exc.response.reason_phrase})"
        raise ValueError(
...
async def encode_image_to_base64_async(
    ...
    if isinstance(image_source, (str, Path)):
        ...
        if source.startswith(("http://", "https://")):
            ...
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    response = await client.get(source, headers=default_headers)
                    response.raise_for_status()
            ...
            # File I/O is fast enough for most cases, but could use aiofiles for massive files
...
# ──────────────────────────────────────────────────────────────────────────────
# Encapsulated Tool Execution with Observability (Async-Aware)
# ──────────────────────────────────────────────────────────────────────────────


async def execute_tool_with_span_async(
    tool_name: str,
    ...
    *,
    ...
    with tracer.start_as_current_span(
        ...
        try:
            # ✅ KEY CHANGE: Support both sync and async tool executors
...
# ──────────────────────────────────────────────────────────────────────────────
# Core Async Streaming Completion
# ──────────────────────────────────────────────────────────────────────────────
    ...
    *,
    ...
    with tracer.start_as_current_span(span_name) as loop_span:
        ...
        if current_messages is None:
            if image_source:
                t0 = time.perf_counter()
                base64_img, mime_type = await encode_image_to_base64_async(image_source)
                logger.info(
                ...
                if (
                    ...
                    if structured.validator_backend:
                        span.set_attribute(
                            ...
                            structured.validator_backend,
                        )
                    span.set_attribute(
                        "llm.structured_output.success", structured.success
                    )
                    span.set_attribute(
                        "llm.structured_output.format", structured.format_used.value
                    )
...
async def run_generate_stream_async(
    ...
    *,
    ...
    """Async stream a raw text completion with Phoenix observability."""
    if project_name:
        setup_observability(
            project_name=project_name,
            capture_content=capture_content,
    ...
    with tracer.start_as_current_span("text_completion") as span:
        ...
        logger.info("─" * 60)
        logger.info(f"📝 Async Text Completion Mode")
        logger.info(f"🤖 Model        : {model}")
        ...
        try:
            ...
            span.record_exception(exc)
            span.set_status(Status(StatusCode.ERROR))
            logger.exception("❌ Async text streaming failed")
            raise
        finally:
            console.print()
            total_secs = time.perf_counter() - t_request_start
            ...
            logger.info("─" * 60)
            logger.info(f"📊 Async Generation summary")
            if usage:
                tok_per_sec = (
...
def get_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Async stream vision-model chat completions with Phoenix observability."
    )
    parser.add_argument(
        "prompt",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path or URL to an image to analyze.",
    )
    parser.add_argument("--project", type=str, default="achat-stream-obs")
