...
class Tokenizer(Pipeline):
    ...
    def __init__(self, lowercase=True, emoji=True, alphanum=False, stopwords=False):
        ...
        if alphanum:
            ...
            self.segment = regex.compile(
                rf"[{pattern}](?:\B\S)*", flags=regex.WORD)

        # Stop words
        self.stopwords = stopwords if isinstance(
            stopwords, list) else Tokenizer.STOP_WORDS if stopwords else False
    ...
    def __call__(self, text):
        ...
        if self.alphanum:
            ...
            tokens = [token for token in tokens if re.match(
                self.alphanum, token)]
        else:
            # Text segmentation per Unicode Standard Annex #29
