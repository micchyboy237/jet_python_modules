...
class HTMLToMarkdown(Pipeline):
    ...
    def __init__(self, paragraphs=False, sections=False):
        ...
        if not SOUP:
            raise ImportError(
                'HTMLToMarkdown pipeline is not available - install "pipeline" extra to enable')
    ...
    def metadata(self, node):
        ...
        metadata = [
            f"**{title.text.strip()}**"] if title and title.text else []
    ...
    def default(self, soup):
        ...
        for line in soup.get_text().split("\n"):
            ...
            lines.append(f"\f{line}" if self.sections and re.search(
                r"^#+ ", line) else line)
    ...
    def table(self, node, article):
        ...
        for row in rows:
            ...
            elements.append(
                f"|{'|'.join(self.process(column, article) for column in columns)}|")
    ...
    def articletext(self, node, text):
        ...
        valid = node.name in valid or (
            node.parent and node.parent.name in ("th", "td"))

        # Check if text is valid article text
        text = text if (valid or self.isheader(
            node)) and not self.islink(node) else ""
        if text:
