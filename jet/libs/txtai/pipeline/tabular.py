...
class Tabular(Pipeline):
    ...
    def __init__(self, idcolumn=None, textcolumns=None, content=False):
        ...
        if not PANDAS:
            raise ImportError(
                'Tabular pipeline is not available - install "pipeline" extra to enable')
    ...
    def process(self, df):
        ...
        for index, row in df.iterrows():
            ...
            if isinstance(self.content, list):
                row = {column: self.column(value) for column, value in row.to_dict(
                ).items() if column in self.content}
                rows.append((uid, row, None))
            elif self.content:
                row = {column: self.column(value)
