...
class FileToHTML(Pipeline):
    ...
    def __init__(self, backend="available"):
        ...
        self.backend = Tika() if backend == "tika" else Docling(
        ) if backend == "docling" else None
...
class Tika:
    ...
    def available():
        ...
        try:
            _ = Popen(path, stdout=open(os.devnull, "w"),
    ...
    def __init__(self):
        ...
        if not Tika.available():
            raise ImportError(
                'Tika engine is not available - install "pipeline" extra to enable. Also check that Java is available.')
...
class Docling:
    ...
    def __init__(self):
        ...
        if not Docling.available():
            raise ImportError(
                'Docling engine is not available - install "pipeline" extra to enable')
    ...
    def __call__(self, path):
        ...
        html = self.converter.convert(
            path).document.export_to_html(html_head="<head/>")
    ...
    def normalize(self, html):
        ...
        html = html.replace(
            "<head/>", "<head/><body>").replace("</html>", "</body></html>")
