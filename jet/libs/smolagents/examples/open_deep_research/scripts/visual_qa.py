...
class VisualQAGPT4Tool(Tool):
    name = "visual_qa_gpt4"
    description = (
        "Answer questions about the content of an image using GPT-4o vision capabilities. "
        "Ideal for describing images, reading text in images, understanding charts, etc."
    )
    inputs = {
        "image_path": {
            "type": "string",
            "description": "Local path to the image file (must be already downloaded)",
        },
        "question": {
            "type": "string",
            "description": "The question you want answered about the image",
            "nullable": True,
        },
    }
    output_type = "string"
    ...
    def forward(self, image_path: str, question: str | None = None) -> str:
        ...
        if not question:
            question = "Provide a very detailed description of this image, including any text, numbers, layout, and visual elements."
        base64_image = encode_image(image_path)
        mime_type, _ = mimetypes.guess_type(image_path)
        if not mime_type:
        ...
        try:
            ...
            return f"Visual QA failed: {str(e)}\nImage path was: {image_path}"
