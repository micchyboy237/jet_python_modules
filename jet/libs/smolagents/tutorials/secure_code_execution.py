...
def demo_01_local_code_agent_default_executor():
    ...
    model = create_llm_model(temperature=0.68, max_tokens=3072)
...
def demo_02_local_with_custom_authorized_imports():
    ...
    model = create_llm_model(temperature=0.7, max_tokens=4096)
...
def demo_04_local_model_e2b_sandbox():
    ...
    model = create_llm_model(temperature=0.72, max_tokens=4096)

    # executor_type="e2b" → code runs in E2B cloud sandbox
    with CodeAgent(
        model=model, tools=[], executor_type="e2b", name="e2b-protected-agent"
    ) as agent:
        result = agent.run(
            "Calculate 2**1000 and return only the number of digits in the result."
        )
...
def demo_05_inference_client_with_modal_executor():
    ...
    model = InferenceClientModel(
        model_id="Qwen/Qwen2.5-72B-Instruct", temperature=0.7, max_tokens=4096
    )

    with CodeAgent(
        model=model, tools=[], executor_type="modal", name="modal-secure-compute"
    ) as agent:
        result = agent.run(
            "What is the 35th number in the Tribonacci sequence (T_n = T_{n-1} + T_{n-2} + T_{n-3})?"
        )
        print("Result:", result)
...
def demo_06_local_model_docker_executor_simple():
    ...
    with CodeAgent(
        ...
        result = agent.run(
            "What is the current number one programming language according to recent rankings?"
        )
        print("Result:", result)
...
def demo_07_wasm_executor_local_model():
    ...
    model = create_llm_model(temperature=0.68)

    # executor_type="wasm" – very strong isolation, no OS access
    agent = CodeAgent(
        model=model, tools=[], executor_type="wasm", name="wasm-sandboxed-agent"
    )

    result = agent.run(
        "Compute sum of squares of first 100 natural numbers using list comprehension."
    )
    print("Result:", result)
