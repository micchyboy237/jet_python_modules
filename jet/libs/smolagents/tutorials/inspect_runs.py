...
from smolagents import (
    CodeAgent,
    InferenceClientModel,  # assuming this is the compatible client for local openai-like servers
    ToolCallingAgent,
    VisitWebpageTool,
    WebSearchTool,
)
...
def demo_01_local_code_agent_basic():
    ...
    model = create_llm_model(temperature=0.75, max_tokens=3072)
...
def demo_02_local_toolcalling_agent():
    ...
    model = create_llm_model(temperature=0.6, max_tokens=4096)
    ...
    result = agent.run(
        "What was the closing price of TSLA on its most recent trading day?"
    )
    print("Result:", result)
...
def demo_03_local_manager_with_subagent():
    ...
    manager = CodeAgent(
        model=model, managed_agents=[search_agent], tools=[], name="coordinating_agent"
    )
...
def demo_05_local_phoenix_tracing():
    ...
    agent = CodeAgent(
        model=model,
        managed_agents=[
            ToolCallingAgent(tools=[WebSearchTool()], model=model, name="fast-lookup")
        ],
    )
