...
def build_topic_model_with_custom_components(
    ...
    topic_model = BERTopic(
        ...
        hdbscan_model=clustering_model_obj
        if clustering_model.lower() == "hdbscan"
        else None,
...
def compare_model_configurations(
    ...
    for i, config in enumerate(configs):
        try:
            ...
            results[f"config_{i}"] = {"error": str(e), "config": config}
...
if __name__ == "__main__":
    ...
    model_default, topics_default, probs_default = (
        build_topic_model_with_custom_components(docs, calculate_probabilities=True)
    )
    print(f"Default model - Topics: {len(model_default.get_topic_info())}")
    print(
        f"Outlier percentage: {(topics_default.count(-1) / len(topics_default)) * 100:.1f}%"
    )
    ...
    model_custom, topics_custom, probs_custom = (
    ...
    print(
        f"Outlier percentage: {(topics_custom.count(-1) / len(topics_custom)) * 100:.1f}%"
    )

    print("\n=== Using KMeans Clustering ===")
    model_kmeans, topics_kmeans, probs_kmeans = (
        build_topic_model_with_custom_components(
            docs,
    ...
    print(
        f"Outlier percentage: {(topics_kmeans.count(-1) / len(topics_kmeans)) * 100:.1f}%"
    )
    ...
    for config_name, result in comparison.items():
        if "error" in result:
            ...
            print(
                f"  {config_name}: {result['n_topics']} topics, {result['outlier_percentage']:.1f}% outliers"
            )
