        ...
        if value:  # Check if not empty
            ...
            if key in ["current_topics", "original_topics"]:
                value_list = [
                    TopicComparisonEntry(
                        topic_id=topic_id,
                        words=[TopicRepresentation(word=word, score=score) for word, score in (words if words else [])]
                    )
                    for topic_id, words in value.items()
                ]
            else:
                value_list = value  # Keep n_topics (int) and changed_topics (list[int]) as is
            save_file(
                {
