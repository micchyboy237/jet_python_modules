...
def example_get_unique_distances():
    ...
    unique_dists = get_unique_distances(
        np.array(distances, dtype=float), noise_max=noise_max
    )
...
def example_select_topic_representation():
    ...
    logging.info(
        "Selecting topic representation with topic_embeddings and use_ctfidf=False..."
    )
    ...
    logging.info(
        "Selecting topic representation with ctfidf_embeddings and use_ctfidf=True..."
    )
    repr_, ctfidf_used = select_topic_representation(
        ctfidf_embeddings, None, use_ctfidf=True
    )
    logging.info(f"Selected representation: {repr_}, ctfidf_used: {ctfidf_used}")

    logging.info("Selecting sparse ctfidf representation with output_ndarray=False...")
    repr_, ctfidf_used = select_topic_representation(
        ctfidf_embeddings_sparse, None, use_ctfidf=True, output_ndarray=False
    )
    logging.info(
        f"Selected representation type: {type(repr_)}, ctfidf_used: {ctfidf_used}"
    )
