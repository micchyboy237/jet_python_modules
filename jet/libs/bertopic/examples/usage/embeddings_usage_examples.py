...
def example_single_document_embedding():
    ...
    print(
        f"Min value: {np.min(single_embedding):.2f}, Max value: {np.max(single_embedding):.2f}"
    )
...
def example_compare_embeddings():
    ...
    print(
        f"Embeddings are equal: {np.array_equal(bertopic_embeddings, sentence_embeddings)}"
    )
