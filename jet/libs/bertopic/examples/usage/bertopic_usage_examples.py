                ...
                "topics": [
                    {
                        "topic_id": topic_id,
                        "top_words": [
                            {"word": word[0], "weight": float(word[1])}
                            for word in topic_model.get_topic(topic_id)[:top_n_words]
                        ],
        ...
        formatted_data = [
            {
                ...
                "top_words": [
                    {"word": word[0], "weight": float(word[1])}
                    for word in topic_model.get_topic(row["Topic"])[:top_n_words]
                ],
                "representative_docs": topic_model.get_representative_docs(
                    row["Topic"]
                )[:3],
...
def get_vectorizer(total_docs: int) -> CountVectorizer:
    ...
    min_df = 2
    max_df = min(
        0.95, max(0.5, (total_docs - 1) / total_docs)
    )  # Ensure max_df is valid
...
def example_base_topic_modeling():
    ...
    topic_model = BERTopic(
        vectorizer_model=get_vectorizer(len(documents)), verbose=True
    )
...
def example_topic_prediction():
    ...
    for i, (topic, prob) in enumerate(zip(topics, probs)):
        words = topic_model.get_topic(topic)[:5]
        logger.info(
            f"Document {i + 1} assigned to Topic {topic} (Probability: {prob:.2f}): "
...
def example_hierarchical_topics():
    ...
    if not documents:
        ...
        return None
    topic_model = BERTopic(
        vectorizer_model=get_vectorizer(len(documents)), min_topic_size=10, verbose=True
    )
    logger.info("Fitting topic model...")
    topic_model.fit(documents)
...
def example_topic_search():
    ...
    for topic, sim in zip(similar_topics, similarity):
        words = topic_model.get_topic(topic)[:5]
        logger.info(
            f"Topic {topic} (Similarity: {sim:.2f}): "
            f"{', '.join([word[0] for word in words])}"
        )
