...
VAD_MODEL = (
    "/Users/jethroestrada/.cache/pretrained_models/sherpa-onnx/vad/ten-vad.onnx"
)
...
def main():
    ...
    try:
        ...
        filename_for_speech = OUTPUT_DIR / time.strftime("%Y%m%d-%H%M%S-speech.wav")
        sf.write(str(filename_for_speech), speech_samples, samplerate=sample_rate)

        filename_for_all = OUTPUT_DIR / time.strftime("%Y%m%d-%H%M%S-all.wav")
