...
def get_scenario_env(token_provider: Optional[Callable[[], str]] = None, env_file: str | None = None) -> Dict[str, str]:
    ...
    Args:
        token_provider: Optional callable to provide Azure token.
        env_file: The path to the env_file to read. (if None, default to DEFAULT_ENV_FILE)
    ...
    env.update({k: str(v) for k, v in env_file_contents.items()})
    return env
...
export PYTHONPATH="/Users/jethroestrada/.pyenv/versions/3.12.9/lib/python3.12/site-packages:\
/Users/jethroestrada/Desktop/External_Projects/Jet_Projects/jet_python_modules/jet:\
/Users/jethroestrada/Desktop/External_Projects/Jet_Projects/jet_python_modules/jet/shared_modules/shared:$PYTHONPATH"
...
"""
    ...
    if autogen_repo_base is None:
        ...
        if autogen_repo_base is None:
            raise ValueError(
                "Could not find AutoGen repo base. Please set the environment variable AUTOGEN_REPO_BASE to the root of the AutoGen repository (e.g., /path/to/autogen)."
            )
    if not os.path.isdir(autogen_repo_base):
        raise ValueError(
            f"AutoGen repo base path '{autogen_repo_base}' does not exist or is not a directory. Please set AUTOGEN_REPO_BASE to the correct path (e.g., /path/to/autogen)."
        )
    ...
    # Mount jet package to resolve ModuleNotFoundError
    jet_package_path = "/Users/jethroestrada/Desktop/External_Projects/Jet_Projects/jet_python_modules/jet"
    if os.path.isdir(jet_package_path):
        volumes[str(pathlib.Path(jet_package_path).absolute())] = {"bind": "/usr/local/lib/python3.11/site-packages/jet", "mode": "rw"}
        print(f"DEBUG: Mounting jet package '{jet_package_path}' to '/usr/local/lib/python3.11/site-packages/jet'")
    else:
        print(f"Warning: Jet package path '{jet_package_path}' not found. ModuleNotFoundError may occur if 'jet' module is required.")
    # Mount shared module to resolve ModuleNotFoundError
    shared_package_path = "/Users/jethroestrada/Desktop/External_Projects/Jet_Projects/jet_python_modules/jet/shared_modules/shared"
    if os.path.isdir(shared_package_path):
        volumes[str(pathlib.Path(shared_package_path).absolute())] = {"bind": "/usr/local/lib/python3.11/site-packages/shared", "mode": "rw"}
        print(f"DEBUG: Mounting shared module '{shared_package_path}' to '/usr/local/lib/python3.11/site-packages/shared'")
    else:
        print(f"Warning: Shared module path '{shared_package_path}' not found. ModuleNotFoundError may occur if 'shared' module is required.")
    docker_host = os.environ.get("DOCKER_HOST", "unix:///var/run/docker.sock")
    print(f"DEBUG: DOCKER_HOST set to '{docker_host}'")
    if docker_host.startswith("unix://"):
        docker_socket = os.path.abspath(docker_host[7:])
        print(f"DEBUG: Attempting to mount Docker socket '{docker_socket}'")
        if ".colima" in docker_socket:
            print(f"Warning: Detected Colima Docker socket '{docker_socket}'. Skipping socket mount due to known compatibility issues.")
            ...
            try:
                ...
                if stat.S_ISSOCK(st_mode):
                    ...
                    print(f"DEBUG: Docker socket '{docker_socket}' is valid and will be mounted")
                else:
                    print(f"Warning: Path '{docker_socket}' exists but is not a socket. Skipping socket mount.")
            except OSError as e:
                print(f"Warning: Failed to access Docker socket '{docker_socket}': {e}. Skipping socket mount.")
        else:
            print(f"Warning: Docker socket '{docker_socket}' not found. Skipping socket mount.")
    print("Mounting:")
    ...
    try:
        ...
        raise
    docker_timeout: float = timeout + 60
