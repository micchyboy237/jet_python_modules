...
async def output_result(_agent: ClosureContext, message: StreamResult, ctx: MessageContext) -> None:
    queue = cast(asyncio.Queue[StreamResult], cl.user_session.get(
...
async def start_chat() -> None:
    ...
    tools: List[Tool] = [FunctionTool(
        get_weather, description="Get weather tool.")]
    ...
    await ClosureAgent.register_closure(
        runtime, CLOSURE_AGENT_TYPE, output_result, subscriptions=lambda: [
            TypeSubscription(topic_type=TASK_RESULTS_TOPIC_TYPE, agent_type=CLOSURE_AGENT_TYPE)]
    )
...
async def chat(message: cl.Message) -> None:
    ...
    runtime = cast(SingleThreadedAgentRuntime,
                   cl.user_session.get("run_time"))  # type: ignore
    queue = cast(asyncio.Queue[StreamResult], cl.user_session.get(
        "queue_stream"))  # type: ignore

    task1 = asyncio.create_task(runtime.send_message(UserMessage(
        content=message.content, source="User"), AgentId("weather_agent", "default")))
