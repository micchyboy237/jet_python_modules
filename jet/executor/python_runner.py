...
def run_python_files_in_directory(
    ...
    # Copy previous statuses (preserve order)
    new_status_data = status_data.copy()
    known_files = {entry["file"] for entry in new_status_data}
    ...
    for file_path in files:
        ...
        # --- Update existing or append new (preserve order) ---
        found = False
        for entry in new_status_data:
            if entry["file"] == str(rel_path):
        ...
        if success_dir and failed_dir:
            ...
            # Remove old failed log on success
            if process.returncode == 0:
