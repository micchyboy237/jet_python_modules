...
from typing import AsyncIterator, Dict, List, Literal, Optional, TypedDict
...
PLAYWRIGHT_CHROMIUM_EXECUTABLE = "/Users/jethroestrada/Library/Caches/ms-playwright/chromium-1208/chrome-mac/Chromium.app/Contents/MacOS/Chromium"


class FileInput(BaseModel):
    file_path: str = Field(..., description="Path to the file (e.g., 'example.txt')")
    encoding: Literal["utf-8", "ascii"] = Field("utf-8", description="File encoding")
...
class UrlOutput(BaseModel):
    ...
    title: Optional[str] = Field(None, description="Page title or error message")
...
class SearchTextsInput(BaseModel):
    texts: List[str] = Field(..., description="List of text strings to search through")
    ...
    chunk_size: int = Field(500, description="Size of text chunks", ge=50, le=1000)
    chunk_overlap: int = Field(100, description="Overlap between chunks", ge=0, le=200)
...
async def summarize_text(
    arguments: SummarizeTextInput, ctx: Context
) -> SummarizeTextOutput:
...
@server.tool(
    description="Search through a list of texts using semantic similarity.",
    annotations={"audience": ["assistant"], "priority": 0.75},
)
async def search_texts_tool(
    arguments: SearchTextsInput, ctx: Context
) -> SearchTextsOutput:
    ...
    try:
        ...
        await ctx.report_progress(
            100, 100, f"Found {len(results)} matching text chunks"
        )
