...
def test_merge_duplicate_texts_agglomerative(capsys):
    ...
    # Mock SentenceTransformer to return controlled embeddings
    with patch("sentence_transformers.SentenceTransformer") as MockModel:
        ...
        mock_model.encode.side_effect = lambda headers, **kwargs: np.array([
            ...
            [0.95, 0.05],  # "Intro" (similar to Introduction)
            [0.0, 1.0]  # "Conclusion" (distinct)
        ])
    ...
    # Check that similar headers ("Introduction" and "Intro") are merged under "Introduction"
    merged_text = next(
    ...
    # Check that the distinct header ("Conclusion") is preserved
...
def test_merge_duplicate_texts_agglomerative_with_markdown(capsys):
    # Sample input data with Markdown headers
    texts = [
        {
            "text": "## Introduction\nThis is the first introduction.",
            "doc_index": 0,
...
def test_merge_duplicate_texts_agglomerative_multiple_markdown_headers(capsys):
    # Sample input data with multiple Markdown headers to merge
    texts = [
        {
            "text": "## Introduction\nThis is the first introduction.",
            "doc_index": 0,
    ...
    with patch("sentence_transformers.SentenceTransformer") as MockModel:
        ...
        mock_model.encode.side_effect = lambda headers, **kwargs: np.array([
            [1.0, 0.0],      # "## Introduction"
            [0.95, 0.05],    # "## Intro" (similar)
            [0.97, 0.03],    # "## Overview" (similar)
            [0.0, 1.0]       # "## Conclusion" (distinct)
        ])
    ...
    # Check that similar headers ("## Introduction", "## Intro", "## Overview") are merged under "## Introduction"
    merged_text = next(
        (t for t in result if t["header"] == "## Introduction"), None)
    assert merged_text is not None, "Merged text with header '## Introduction' not found"
    assert "This is the first introduction." in merged_text[
