            ...
            for idx in range(0, len(nodes), self.choice_batch_size):
                ...
                if isinstance(result, str):
                    ...
                    initial_results.extend(
                        [NodeWithScore(node=node, score=0.0)
                ...
                for choice_node, relevance, doc in zip(choice_nodes, relevances, result.documents):
                    score = relevance
                    metadata = {
                        **choice_node.metadata,
                        "feedback": doc.feedback
                    }
                    node_dict = {
                        **make_serializable(choice_node),
                        "metadata": metadata
                    }
                    node = TextNode(**node_dict)
