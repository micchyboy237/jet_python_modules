...
def collect_header_chunks(
    ...
    context_size = get_model_max_tokens(tokenizer_model) if tokenizer_model else 512
    ...
    for header_doc in header_docs:
        ...
        headers_context_processed = (
            preprocess_text(headers_context_text) if headers_context_text else ""
        )
        ...
        chunks = chunk_texts(
            original_content, chunk_size, chunk_overlap, tokenizer_model, header_buffer
        )
        ...
        for chunk in chunks:
            if chunk.strip():
                ...
                start_idx = (
                    ...
                    if end_idx - start_idx > chunk_overlap
                    else start_idx
                )
        ...
        for next_chunk in chunks[1:]:
            ...
            if next_start <= end_idx:
                ...
                preprocessed_content += (
                    " " + next_preprocessed_content[overlap:]
                    if overlap > 0
                    else next_preprocessed_content
                )
                end_idx = new_end
...
def search_headers(
    ...
    all_vectors = client.get_embeddings(
        all_texts, return_format="numpy", batch_size=32, show_progress=True
    )
    ...
    for i, (
        ...
        weighted_sim, header_content_sim, headers_sim, content_sim = (
