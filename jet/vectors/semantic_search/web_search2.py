...
from jet.adapters.llama_cpp.hybrid_search import (
    HybridSearchResult as BaseHybridSearchResult,
)
...
from jet.scrapers.utils import SearchResult, search_data
...
# We reuse the hybrid search result structure, but now the result dicts are generic (not HeaderSearchResult),
# and header_docs can be empty or removed.


class HybridSearchResult(BaseHybridSearchResult):
    source: str
    original_order: int  # optional, for stable sort
    mtld: float | None
    mtld_category: str | None
    ltr_ratio: float | None


class HybridSearchRawResult(TypedDict):
    query: str
    search_engine_results: list[dict]
    started_urls: list[str]
    completed_urls: list[str]
    header_docs: list[dict]  # can be left empty or omitted
    all_search_results: list[
        dict
    ]  # generic dicts after hybrid search, not HeaderSearchResult
    filtered_results: list[dict]  # generic dicts after final filtering
    grouped_context: str
...
def searxng_search(query: str, *, use_cache: bool = False) -> list[SearchResult]:
    """
    Perform a synchronous search using SearXNG.
    """
    # For now we delegate to the existing implementation
    results = search_data(query, use_cache=use_cache)

    # Basic validation (optional but recommended for tools)
    ...
    for item in results:
        if not isinstance(item, dict) or "url" not in item:
            raise ValueError(
                "Each search result must be a dict containing at least 'url'"
            )
...
async def hybrid_search(
    ...
    *,
    ...
    Version refactored for modern retriever workflow.
    """

    search_engine_results = searxng_search(query, use_cache=use_cache)
    urls = [r["url"] for r in search_engine_results][:urls_limit]

    collected_htmls: dict[str, str | None] = {}
    statuses: dict[str, ScrapeStatus | str] = {}

    # No longer using HeaderDoc/HeaderSearchResult. These now act as generic dicts/lists.
    header_docs: list = []
    ...
    all_htmls_with_status: dict[str, dict] = {}
    ...
    for url in all_completed_urls:
        ...
        # Convert HTML to sentences (new approach)
        sentences = html_to_sentences(html)
        if not sentences:
            logger.warning(f"No sentences extracted from {url}")
            continue

        # Per-page hybrid search
        hybrid = HybridSearch.from_documents(
            documents=sentences,
            model=embed_model,
            min_hybrid_score=0.018,
        )

        page_results_raw = hybrid.search(
            query=query,
            top_k=80,
        ...
        page_results: list[HybridSearchResult] = []
        for i, raw in enumerate(page_results_raw):
            text = raw["text"]
            ...
            enriched: HybridSearchResult = {
                **raw,
                ...
                "original_order": i,
                "mtld": mtld,
                "mtld_category": mtld_cat,
                "ltr_ratio": ltr,
            }
            # Use new hybrid score for filtering
            if raw["hybrid_score"] >= MEDIUM_QUALITY_SCORE and mtld_cat != "very_low":
                page_results.append(enriched)

        # For compatibility, attach page-level meta as 'header_docs'. Retain blank list if unwanted.
        header_docs.extend([])

        # Accumulate results
        search_results.extend(page_results)

        sub_total_tokens = sum(count_tokens(r["text"], llm_model) for r in page_results)
        sub_high = sum(
            count_tokens(r["text"], llm_model)
            for r in page_results
            if r["hybrid_score"] >= HIGH_QUALITY_SCORE
        )
        sub_medium = sum(
            count_tokens(r["text"], llm_model)
            for r in page_results
            if MEDIUM_QUALITY_SCORE <= r["hybrid_score"] < HIGH_QUALITY_SCORE
        )

        sub_mtld_values = [
            r["mtld"] for r in page_results if r["hybrid_score"] >= HIGH_QUALITY_SCORE
        ]
        sub_mtld_avg = (
    ...
    # Stable sort: by hybrid_score desc, then (optionally) by source order, then by original position in source
    search_results.sort(
        key=lambda x: (
            -x["hybrid_score"],
            # Optionally respect a sorted_url_stats if exists, else fallback
            x.get("source_order", 9999),
            x.get("original_order", 9999),
        )
    )
    for i, r in enumerate(search_results, 1):
        r["rank"] = i
        r["score"] = r["hybrid_score"]  # For compatibility

    # URL stats (new logic)
    url_stats = defaultdict(lambda: {"high": 0, "medium": 0, "count": 0})
    for r in search_results:
        src = r.get("source", "unknown")
        tok = count_tokens(r["text"], llm_model)
        if r["hybrid_score"] >= HIGH_QUALITY_SCORE:
            url_stats[src]["high"] += tok
            url_stats[src]["count"] += 1
        elif r["hybrid_score"] >= MEDIUM_QUALITY_SCORE:
            url_stats[src]["medium"] += tok
            url_stats[src]["count"] += 1

    sorted_url_stats = sorted(
        (
            {
                "url": url,
    ...
    for result in search_results:  # already sorted by score
        tokens = count_tokens(result["text"], llm_model)
