...
def mock_sentence_transformer():
    with patch(
        "jet.vectors.semantic_search.file_vector_search.SentenceTransformerRegistry"
    ) as mock_registry:
        mock_model = Mock()
        mock_model.encode.side_effect = (
            lambda x, **kwargs: np.array([0.1, 0.2, 0.3])
            if isinstance(x, str)
            else np.array([[0.1, 0.2, 0.3]] * len(x))
        )
...
def test_collect_file_chunks_single_file(temp_file):
    ...
    assert file_paths[0] == expected_file_path, (
        f"Expected file path {expected_file_path}, got {file_paths[0]}"
    )
    assert file_names[0] == expected_file_name, (
        f"Expected file name {expected_file_name}, got {file_names[0]}"
    )
    assert parent_dirs[0] == expected_parent_dir, (
        f"Expected parent directory {expected_parent_dir}, got {parent_dirs[0]}"
    )
    assert len(chunks) == 1, "Expected exactly one chunk"
    assert chunks[0][0] == expected_chunk[0], (
        f"Expected chunk file path {expected_chunk[0]}, got {chunks[0][0]}"
    )
    assert chunks[0][1].rstrip() == expected_chunk[1], (
        f"Expected chunk content {expected_chunk[1]}, got {chunks[0][1]}"
    )
    assert chunks[0][2] == expected_chunk[2], (
        f"Expected chunk start index {expected_chunk[2]}, got {chunks[0][2]}"
    )
    assert chunks[0][3] == expected_chunk[3], (
        f"Expected chunk end index {expected_chunk[3]}, got {chunks[0][3]}"
    )
    assert chunks[0][4] == expected_chunk[4], (
        f"Expected chunk num_tokens {expected_chunk[4]}, got {chunks[0][4]}"
    )
    ...
    assert chunks[0][4] == expected_chunk_custom[4], (
        f"Expected chunk num_tokens {expected_chunk_custom[4]} with custom tokenizer, got {chunks[0][4]}"
    )
...
def test_collect_file_chunks_with_extensions(tmp_path):
    ...
    file_paths, _, _, _ = collect_file_chunks(str(tmp_path), extensions=[".txt"])
...
def test_search_files(mock_sentence_transformer, temp_file):
    ...
    def custom_tokenizer(text):
        return len(text.split())  # Simple space-based tokenizer
    ...
    results = list(search_files(temp_file, query, extensions=[".txt"], top_k=top_k))
    ...
    assert result["metadata"]["chunk_idx"] == 0, (
        "Expected chunk_idx to be 0 for single chunk"
    )
    ...
    results = list(
        search_files(
            temp_file, query, extensions=[".txt"], top_k=top_k, split_chunks=True
        )
    )
...
def test_search_files_no_results(mock_sentence_transformer, tmp_path):
    ...
    results = list(search_files(str(tmp_path), "test query", extensions=[".bin"]))
...
def test_search_files_chunking(temp_file):
    ...
    assert results_default[0]["metadata"]["num_tokens"] == expected_tokens_merged, (
        f"Expected {expected_tokens_merged} tokens, got {results_default[0]['metadata']['num_tokens']}"
    )
    assert results_default[0]["metadata"]["chunk_idx"] == 0, (
        "Expected chunk_idx to be 0 for merged result"
    )
    ...
    assert (
        results_custom[0]["metadata"]["num_tokens"] == expected_tokens_merged_custom
    ), (
        f"Expected {expected_tokens_merged_custom} tokens with custom tokenizer, got {results_custom[0]['metadata']['num_tokens']}"
    )
    ...
    assert all(
        r["metadata"]["end_idx"] - r["metadata"]["start_idx"] <= chunk_size
        for r in results_split
    )
    assert all(isinstance(r["metadata"]["chunk_idx"], int) for r in results_split), (
        "All chunk_idx should be integers"
    )
    assert [r["metadata"]["chunk_idx"] for r in results_split] == list(
        range(len(results_split))
    ), (
        f"Expected chunk indices 0 to {len(results_split) - 1}, got {[r['metadata']['chunk_idx'] for r in results_split]}"
    )
    for r in results_split:
        expected_tokens = len(re.findall(r"\b\w+\b|[^\w\s]", r["code"]))
        assert r["metadata"]["num_tokens"] == expected_tokens, (
            f"Expected {expected_tokens} tokens, got {r['metadata']['num_tokens']}"
        )
...
def test_search_files_with_threshold_and_yielding(mock_sentence_transformer, temp_file):
    ...
    def assert_result_structure(
        result, expected_content, expected_file_path, expected_num_tokens
    ):
        assert isinstance(result, dict), "Result should be a dictionary"
        assert result["rank"] == 1, "Rank should be 1 after sorting"
        assert result["score"] >= expected_threshold, (
            f"Score {result['score']} should meet threshold {expected_threshold}"
        )
        assert result["code"].rstrip() == expected_content, (
            f"Expected content {expected_content}, got {result['code']}"
        )
        assert result["metadata"]["file_path"] == expected_file_path, (
            f"Expected file path {expected_file_path}, got {result['metadata']['file_path']}"
        )
        assert result["metadata"]["chunk_idx"] == 0, (
            "Expected chunk_idx to be 0 for single chunk"
        )
        for key in ["name_similarity", "dir_similarity", "content_similarity"]:
            assert isinstance(result["metadata"][key], float), f"{key} should be float"
        assert result["metadata"]["num_tokens"] == expected_num_tokens, (
            f"Expected {expected_num_tokens} tokens, got {result['metadata']['num_tokens']}"
        )
    ...
    assert results[0]["metadata"]["num_tokens"] == expected_num_tokens, (
        f"Expected {expected_num_tokens} tokens with custom tokenizer, got {results[0]['metadata']['num_tokens']}"
    )
...
class TestMergeResults:
    def test_merge_adjacent_chunks(self, tmp_path):
        ...
        assert (
            merged[0]["metadata"]["start_idx"]
            == expected_default[0]["metadata"]["start_idx"]
        )
        assert (
            merged[0]["metadata"]["end_idx"]
            == expected_default[0]["metadata"]["end_idx"]
        )
        assert (
            merged[0]["metadata"]["chunk_idx"]
            == expected_default[0]["metadata"]["chunk_idx"]
        )
        assert abs(merged[0]["score"] - expected_default[0]["score"]) < 1e-10
        assert (
            abs(
                merged[0]["metadata"]["content_similarity"]
                - expected_default[0]["metadata"]["content_similarity"]
            )
            < 1e-10
        )
        assert (
            merged[0]["metadata"]["name_similarity"]
            == expected_default[0]["metadata"]["name_similarity"]
        )
        assert (
            merged[0]["metadata"]["dir_similarity"]
            == expected_default[0]["metadata"]["dir_similarity"]
        )
        assert (
            merged[0]["metadata"]["num_tokens"]
            == expected_default[0]["metadata"]["num_tokens"]
        )
        # Test with custom tokenizer
        merged = merge_results(results, tokenizer=custom_tokenizer)
        assert (
            merged[0]["metadata"]["num_tokens"]
            == expected_custom[0]["metadata"]["num_tokens"]
        )
    ...
    def test_non_adjacent_chunks(self, tmp_path):
        ...
        for i in range(2):
            ...
            assert (
                merged[i]["metadata"]["start_idx"]
                == expected[i]["metadata"]["start_idx"]
            )
            assert (
                merged[i]["metadata"]["end_idx"] == expected[i]["metadata"]["end_idx"]
            )
            assert (
                merged[i]["metadata"]["chunk_idx"]
                == expected[i]["metadata"]["chunk_idx"]
            )
            assert merged[i]["score"] == expected[i]["score"]
            assert (
                merged[i]["metadata"]["content_similarity"]
                == expected[i]["metadata"]["content_similarity"]
            )
    ...
    def test_multiple_files(self, tmp_path):
        ...
        for i in range(2):
            assert merged[i]["code"] == expected[i]["code"]
            assert (
                merged[i]["metadata"]["file_path"]
                == expected[i]["metadata"]["file_path"]
            )
            assert merged[i]["score"] == expected[i]["score"]
