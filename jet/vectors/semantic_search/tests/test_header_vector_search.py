...
def mock_sentence_transformer():
    ...
    with patch(
        "jet.vectors.semantic_search.header_vector_search.SentenceTransformerRegistry"
    ) as mock_registry:
        mock_model = Mock()
...
def test_collect_header_chunks(sample_header_doc):
    ...
    expected_num_tokens_default = len(re.findall(r"\b\w+\b|[^\w\s]", original_content))
    ...
    expected_chunk = (
        ...
        original_content,
        expected_header,
        expected_headers_context,
        0,
        22,
        expected_num_tokens_default,
    )
    expected_chunk_custom = (
        ...
        0,
        22,
        expected_num_tokens_custom,
    )
    result_doc_indices, result_headers, result_headers_context, result_chunks = (
        collect_header_chunks([sample_header_doc])
    )
    ...
    result_doc_indices, result_headers, result_headers_context, result_chunks = (
        collect_header_chunks([sample_header_doc], tokenizer=custom_tokenizer)
    )
...
def test_compute_weighted_similarity_with_content():
    ...
    expected_weighted_sim = (
        (0.3 / 1.0) * expected_header_content_sim
        + (0.3 / 1.0) * expected_headers_sim
        + (0.4 / 1.0) * expected_content_sim
    )
    (
        result_weighted_sim,
        result_header_content_sim,
        result_headers_sim,
        result_content_sim,
    ) = compute_weighted_similarity(
...
def test_search_headers(mock_sentence_transformer, sample_header_doc):
    ...
    assert isinstance(result[0]["metadata"]["header_content_similarity"], float)
    ...
    assert (
        result[0]["metadata"]["preprocessed_headers_context"]
        == expected_preprocessed_headers_context
    )
    assert (
        result[0]["metadata"]["preprocessed_content"] == expected_preprocessed_content
    )
    result = list(
        search_headers(
    ...
    result = list(
        search_headers([sample_header_doc], query, top_k=top_k, split_chunks=True)
    )
    assert len(result) == 1
    assert result[0]["content"] == original_content
    assert result[0]["metadata"]["num_tokens"] == expected_num_tokens_default
...
def test_search_headers_chunking(mock_sentence_transformer):
    ...
    assert (
        result_default[0]["metadata"]["preprocessed_header"]
        == expected_preprocessed_header
    )
    assert (
        result_default[0]["metadata"]["preprocessed_headers_context"]
        == expected_preprocessed_headers_context
    )
    assert (
        result_default[0]["metadata"]["preprocessed_content"]
        == expected_preprocessed_content
    )
    result_custom = list(
        search_headers(
            [header_doc],
    ...
    assert (
        result_custom[0]["metadata"]["preprocessed_header"]
        == expected_preprocessed_header
    )
    assert (
        result_custom[0]["metadata"]["preprocessed_headers_context"]
        == expected_preprocessed_headers_context
    )
    assert (
        result_custom[0]["metadata"]["preprocessed_content"]
        == expected_preprocessed_content
    )
    result_split = list(
        search_headers(
    ...
    assert [r["metadata"]["chunk_idx"] for r in result_split] == list(
        range(len(result_split))
    )
    ...
    assert all(r["metadata"]["parent_header"] == "Parent Header" for r in result_split)
    assert all(
        r["metadata"]["preprocessed_header"] == expected_preprocessed_header
        for r in result_split
    )
    assert all(
        r["metadata"]["preprocessed_headers_context"]
        == expected_preprocessed_headers_context
        for r in result_split
    )
...
class TestMergeResults:
    ...
    def test_merge_adjacent_chunks(self):
        ...
        assert (
            result[0]["metadata"]["start_idx"] == expected[0]["metadata"]["start_idx"]
        )
        assert result[0]["metadata"]["end_idx"] == expected[0]["metadata"]["end_idx"]
        assert (
            result[0]["metadata"]["chunk_idx"] == expected[0]["metadata"]["chunk_idx"]
        )
        assert abs(result[0]["score"] - expected[0]["score"]) < 1e-10
        assert (
            abs(
                result[0]["metadata"]["content_similarity"]
                - expected[0]["metadata"]["content_similarity"]
            )
            < 1e-10
        )
        assert result[0]["metadata"]["header"] == expected[0]["metadata"]["header"]
        assert (
            result[0]["metadata"]["parent_header"]
            == expected[0]["metadata"]["parent_header"]
        )
        assert (
            result[0]["metadata"]["preprocessed_header"]
            == expected[0]["metadata"]["preprocessed_header"]
        )
        assert (
            result[0]["metadata"]["preprocessed_headers_context"]
            == expected[0]["metadata"]["preprocessed_headers_context"]
        )
        assert (
            result[0]["metadata"]["preprocessed_content"]
            == expected[0]["metadata"]["preprocessed_content"]
        )
    ...
    def test_non_adjacent_chunks(self):
        ...
        for i in range(2):
            ...
            assert (
                result[i]["metadata"]["start_idx"]
                == expected[i]["metadata"]["start_idx"]
            )
            assert (
                result[i]["metadata"]["end_idx"] == expected[i]["metadata"]["end_idx"]
            )
            assert (
                result[i]["metadata"]["chunk_idx"]
            ...
            assert (
                result[i]["metadata"]["parent_header"]
                == expected[i]["metadata"]["parent_header"]
            )
            assert (
                result[i]["metadata"]["preprocessed_header"]
                == expected[i]["metadata"]["preprocessed_header"]
            )
            assert (
                result[i]["metadata"]["preprocessed_headers_context"]
                == expected[i]["metadata"]["preprocessed_headers_context"]
            )
            assert (
                result[i]["metadata"]["preprocessed_content"]
                == expected[i]["metadata"]["preprocessed_content"]
            )
    ...
    def test_multiple_headers(self):
        ...
        for i in range(2):
            assert result[i]["content"] == expected[i]["content"]
            assert (
                result[i]["metadata"]["doc_index"]
                == expected[i]["metadata"]["doc_index"]
            )
            assert result[i]["score"] == expected[i]["score"]
            assert result[i]["metadata"]["header"] == expected[i]["metadata"]["header"]
            assert (
