...
def extract_keyword_candidates(
    ...
    if isinstance(texts, str):
        texts = [texts]
    # Preprocess all input texts
    preprocessed_texts = preprocess_texts(texts)
    logger.debug(f"Preprocessed texts: {preprocessed_texts}")
    logger.debug(
        f"Parameters: ngram_range={ngram_range}, stop_words={stop_words}, min_df={min_df}, top_n={top_n}")
    effective_min_df = 1 if len(preprocessed_texts) == 1 else min(min_df, len(preprocessed_texts))
...
def extract_single_doc_keywords(
    ...
    preprocessed_text = preprocess_texts(text)[0]
    ...
    return [{
        ...
        "tokens": _count_tokens(preprocessed_text, nlp),
...
def extract_multi_doc_keywords(
    ...
    result = []
    for i, (text, doc_keywords) in enumerate(zip(preprocessed_texts, keywords)):
...
def extract_keywords_with_candidates(
    ...
    try:
        ...
        keywords = model.extract_keywords(docs=preprocessed_texts, top_n=top_n)
...
def extract_keywords_with_embeddings(
    ...
    doc_embeddings = generate_embeddings(
        preprocessed_texts, model=model_name, return_format="numpy")
    vectorizer = CountVectorizer(ngram_range=keyphrase_ngram_range)
    try:
        vocab = vectorizer.fit(preprocessed_texts).get_feature_names_out()
    except ValueError as e:
        logger.warning(f"Vectorization failed: {e}. Returning empty keywords.")
        return []
    if len(vocab) == 0:
