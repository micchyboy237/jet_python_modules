...
def chunk_texts(
    ...
    show_progress: bool = True,  # New flag to toggle progress bars
) -> List[str]:
    ...
    # Main progress bar over all texts
    text_iter = tqdm(texts, desc="Chunking texts", unit="text") if show_progress else texts
    ...
    for text in text_iter:
        ...
        # Optional progress bar for sentence-level processing
        sentence_iter = tqdm(range(len(sentences)), desc="Processing sentences", unit="sent", leave=False) if show_progress else range(len(sentences))
    ...
    for i, (doc_index, text) in enumerate(zip(doc_indices, texts)):
        ...
        if not strict_sentences and model:
            ...
            for j in range(0, len(tokens), step):
                ...
                left, right = j, min(j + chunk_size - buffer, len(tokens))
                chunk_tokens = []
                chunk_content = ""
                chunk_size_tokens = 0
                is_last_chunk = j + chunk_size - buffer >= len(tokens)
                best_size = 0
                while left <= right:
                    mid = (left + right) // 2
                ...
                if is_last_chunk and not chunk_tokens:
                    # Fallback for last chunk: include remaining tokens
                ...
                if chunk_overlap > 0 and j + len(chunk_tokens) < len(tokens):
                    # Calculate overlap based on actual chunk token count
                    overlap_start = j + len(chunk_tokens) - min(chunk_overlap, len(chunk_tokens))
                    overlap_end = min(j + len(chunk_tokens), len(tokens))
                ...
                chunk_index += 1
            # Merge last chunk if too small, but only if chunk_size > min_chunk_size
...
def chunk_texts_with_data_fast(
    texts: Union[str, List[str]],
    chunk_size: int = 128,
    chunk_overlap: int = 0,
    model: Optional[OLLAMA_MODEL_NAMES] = None,
