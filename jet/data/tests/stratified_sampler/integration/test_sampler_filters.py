...
def test_get_samples_diverse_balanced_from_strings():
    # Given: A list of plain strings with varied content and num_samples=None
    data = [
        "Global markets rise sharply",
        "Tech stocks soar to new highs",
        "Economy slows down significantly",
        "Markets face uncertainty ahead",
        "Stocks rebound quickly today",
        "Financial crisis looms large"
    ]
    sampler = StratifiedSampler(data, num_samples=None)
    expected = [
        'Economy slows down significantly',
        'Financial crisis looms large',
        'Global markets rise sharply',
    ...
    # When: filter_strings is called with n=2 and top_n=None
    with patch("tqdm.tqdm", lambda x, **kwargs: x):
        result = sampler.filter_strings(n=2)
...
def test_filter_strings():
    ...
    data = [
        ProcessedDataString(source="Global markets rise",
                            category_values=["positive", 1, 5.0, True]),
        ProcessedDataString(source="Global markets stabilize",
                            category_values=["positive", 2, 4.8, True]),
