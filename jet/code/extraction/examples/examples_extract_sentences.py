...
def run_examples():
    ...
    text = (
        ...
        "The model must detect semantic boundaries.\nThis is a sentence of a separate block. Another sentence follows in the same paragraph.\n\n"
        
        "Second paragraph starts after a clear separation.\nHere we test if the model respects "
        "existing newlines as strong paragraph cues.\n\n"
        
        "Third and final paragraph. Short but valid. Contains only two sentences."
    )
    sentences = extract_sentences(text, verbose=True)
    print("Example 1 - Basic Usage:")
    save_file(sentences, f"{OUTPUT_DIR}/1_basic_usage_results.json")

    # Example 2.1: Paragraph Segmentation (Low Threshold)
    paragraphs = extract_sentences(text, do_paragraph_segmentation=True, paragraph_threshold=0.1, verbose=True)
    ...
    paragraphs = extract_sentences(text, do_paragraph_segmentation=True, paragraph_threshold=0.5, verbose=True)
    ...
    paragraphs = extract_sentences(text, do_paragraph_segmentation=True, paragraph_threshold=0.9, verbose=True)
    ...
    sentences = extract_sentences(noisy_text, do_paragraph_segmentation=True, paragraph_threshold=0.6, verbose=True)
    ...
    batch_segmented = extract_sentences(texts, verbose=True)
    ...
    batch_segmented = extract_sentences(texts, do_paragraph_segmentation=True, verbose=True)
    print("\nExample 4.2 - Batch Processing (Paragraph Segmentation):")
    ...
    sentences = extract_sentences(legal_text, style_or_domain="ud", language="en", verbose=True)
