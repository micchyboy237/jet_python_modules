...
class TestParseMarkdownMergeParagraphs:
    ...
    def test_merge_consecutive_unordered_lists(self):
        """Test merging consecutive unordered list tokens into a single token."""
        # Given
        input_md = "- Item 1\n- Item 2\n\n- Item 3"
        expected = [
            {
        ...
        # Then
        assert result == expected, f"Expected {expected}, but got {result}"
        assert all(r["content"] == r["content"].strip()
                   for r in result), "Result contains leading/trailing spaces"
        assert all('\uFFFC' not in r["content"]
                   for r in result), "Result contains special characters"
